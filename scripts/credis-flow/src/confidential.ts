 ...
 * `outbe_common::pow`: SHA256(id_be32 || owner || seq_be4 || nonce_be8) must
 * have `POW_DIFFICULTY` (= 1) leading zero bytes. Gem and Nod are exercised
 * once, so `seq` is zero. Difficulty 1 needs ~256 tries.
 */
export function findPowNonce(id: bigint, owner: string): bigint {
  const prefix = concat(be(id, 32), addressBytes(owner), be(0n, 4));
  for (let n = 0n; n < 1_000_000n; n++) {
    const hash = createHash("sha256").update(Buffer.from(concat(prefix, be(n, 8)))).digest();
