...
This file is a canon component, referenced by `gips/GIP-00000.md` section Mechanism. It states the **semantic layer**: the constraint topology and the lexical, verb, and reasoning constraints that govern the prose of every canonical OUTBE artifact and every authored proposal.
...
The authority for these constraints - and for the discipline generally - is the repository alone (see `gips/GIP-00000.md` section Mechanism, "The repository is the sole authority"). No per-environment or per-user state - agent memory, global or repository operator-instruction files - is consulted as a source.

**Domain substrate.** The lexical constraints in section 2 presuppose the OUTBE domain ontology - the token, object, contract, and flow mechanics of the protocol. The section 2 term table carries that vocabulary in compressed form (each term's permitted anchor). The full economic mechanics are an operator-provided domain model; codifying that domain model in full under `canon/` is a separate proposal (see the section "Source-of-truth" note in any signal-source reader and the GIP that introduced this file).

---

## section 1 Constraint topology

### section 1.0 Root constraint: non-propositionality
...
### section 1.1 Ontological constraint
...
### section 1.3 Addressee constraint
...
### section 1.4 Motivation constraint
...
### section 1.5 Anti-persuasion constraint
...
### section 1.6 Non-recovery constraint
...
### section 1.7 Constraint precedence

Constraints are evaluated in order; if a higher constraint is violated, lower-level compliance is irrelevant:

1. Root non-propositionality (section 1.0)
2. Ontological constraint (section 1.1)
3. Agency constraint (section 1.2)
4. Addressee constraint (section 1.3)
5. Motivation constraint (section 1.4)
6. Anti-persuasion constraint (section 1.5)
7. Non-recovery constraint (section 1.6)
8. Lexical and verb-level constraints (section 2, section 3)
9. Reasoning-level forbidden moves (section 4)

### section 1.8 Semantic failure mode
...
**Cost Amount** - Forbidden: percentage of Tribute, fixed fee, premium. Permitted: loaded mining capacity x entry price, in issuance currency.
...
**Floor Price / qualification** - Forbidden: immediate availability, exercisable on issuance, denominated in issuance currency, country-influenceable. Permitted: per-contract threshold (reference-currency entry x 1.08); settlement requires 21 days elapsed AND globally-formed COEN/ref > Floor Price.

**Call event** - Forbidden: automatic settlement, default trigger, forced burn-with-recovery, denominated in issuance currency, country-influenceable. Permitted: per-contract threshold (reference-currency entry x 1.64); forced choice (settle or forfeit) when globally-formed COEN/ref > Call Price for 21 of 30 days; applies to Intex and Gem only.
...
**COEN market cap** - Forbidden: wealth that exists, value held in the system, money in the system, net worth of system. Permitted: price x supply extrapolation; not a realisable aggregate.
...
## section 3 Verb-to-object compatibility
...
## section 4 Forbidden reasoning moves

These govern arguments, distinct from the term-level constraints in section 2 and verb-level constraints in section 3.
...
- Do not equate full consumer enrollment with full Bundle realization. Enrollment is bounded by domestic licensing; Bundle realization is bounded by global Vault capacity. Aggregate-flow analyses must use realized Bundle flow, not enrolled population x per-capita consumption.
...
This document specifies the body shape every GIP and every OIP follows, the registry JSON shape, and the fingerprint algorithm. Referenced from `gips/GIP-00000.md` section 1 (the body schema), section 5 (registry shape), and the canon-tuning loop's dedup step (`canon/authoring-procedure.md` Step 2).
...
| `signal` | required | `{ kind, source, fingerprint, head?, run? }` - the Triggering signal metadata. `kind` is one of: `review-finding`, `run-rubric-gap`, `cross-contract-pattern`, `exploit-feed`, `operator-reject`, `residual-escape`, `canon-deadlock` (operator-only exemption per `gips/GIP-00000.md` section Security Considerations), `bootstrap` (single-use, GIP-00000 only). `head` is **required for every OIP**: the full 40-hex commit SHA of the source repository that the proposal's `file:line` citations and `drift-scope` line ranges are anchored to, so a reader can check out the exact code addressed. `head` is omitted for GIPs - a GIP's citations target the canon in this repository, versioned by this repository's own git |
...
- `review-digest` - single OIP collecting actionable non-Material findings from one contract-review session; one such OIP per contract per session; subject to Atomic/Material exemption per `canon/rubric.md` section "Tier 1 - Admissibility gates"
...
| section  | Section | Required / conditional |
|---|---|---|
...
The two registries (`gips/`, `oips/`) maintain independent `next_id` counters. Identifiers are zero-padded to five digits; the rendered width grows to `max(5, ceil(log10(highest-assigned-id + 1)))` once a registry exceeds 99999 entries.
...
A registry entry whose fingerprint was admitted in a prior run but is absent from the current run undergoes drift-aware resolution: if `git diff <head_at_last_seen> <current_head>` intersects any path in the entry's `drift-scope`, the operator authors a superseding proposal that supersedes the absent entry (and the absent entry's status moves to `Superseded` via the new proposal's `supersedes` field). If the drift-scope is unchanged, the entry stays at its prior status (the candidate is a stochastic miss, not a resolution). The drift detector that flags such entries is design-intent pending tooling (per `canon/discipline.md` section "How the discipline stays current").
