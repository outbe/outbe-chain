...
|---|---|---|---|---|
| L01 | Переход суток | `OFFCHAIN_PENDING` и WorldwideDay FSM согласованы после истечения окна | PASS | `reducer_state_time_region_table_covers_every_persisted_status`, `reducer_is_a_noop_for_processing_states_after_the_process_boundary` и `advance_active_worldwide_days_advances_status_without_creating_or_settling` PASS: reducer сохраняет `OFFCHAIN_PENDING` после process boundary, production advance-path остаётся согласованным. Разрешение границ из `config.outbeProtocol` отдельно покрыто тестами genesis resolver. Полный release E2E остаётся агрегатным доказательством; отдельный core-дубликат с injected state не создавался |
