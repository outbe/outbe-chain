# ADR-B-OCD-002: Define the Tribute and Nod off-chain body boundary

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-15
- **Depends on:** ADR-B-OCD-001

> **Supersession note:** ADR-B-OCD-006 changes the active reset-era body wire/key contract from temporary Postcard plus 32-byte Tribute/Nod IDs to strict-canonical Protobuf plus exact 36-byte `WWD_BE4 || Poseidon_BE32` Tribute/Nod IDs. Nod bucket identity becomes `WWD_BE4 || bucket_key_BE32`. ADR-B-OCD-002 remains authoritative for domain ownership, typed repository boundaries, collection separation, deterministic ordering, pagination semantics, and the off-chain-versus-compact-EVM state split; ADR-B-OCD-006 updates the affected primary/index key suffixes, ID types, codecs, and cursors.

## Context

ADR-B-OCD-001 introduced a domain-neutral off-chain storage facade with equivalent in-memory and MongoDB adapters. Tribute, Nod, Lysis, NodFactory, Gratis, and the precompiles still use their existing EVM-backed records.
...
Add the Serde support required by Postcard directly to these existing records and their nested field types. The same semantic type is temporarily used by both the current EVM schema and the new repository. When the EVM body maps are removed in ADR-B-OCD-005, the domain types remain valid repository records.
...
ADR-B-OCD-002 does not introduce a version envelope. There is no production state requiring backward compatibility, and MongoDB may be deleted and rebuilt after a format change. Canonical commitment encoding and real schema-version semantics belong to ADR-B-OCD-006.
...
ADR-B-OCD-002 distinguishes complete/query records from compact protocol control state.
...
The existing Nod item map, bucket map, owner indexes, and global enumeration remain active until ADR-B-OCD-005.

### MongoDB namespaces

Fix six code-defined namespaces. Because ADR-B-OCD-001 maps namespace values directly to MongoDB collection names, these are also the exact collection names:
...
The concrete repository structs are cloneable handles over the ADR-B-OCD-001 reader capability. Backend substitution already exists below them, so an additional public repository trait would be a shallow seam with only one implementation.
...
ADR-B-OCD-001 guarantees only per-key atomicity. ADR-B-OCD-002 does not pretend that a body plus its indexes is an atomic transaction.
...
Atomic body/index batches and durable projection checkpoints are introduced with the real ExEx consumer in ADR-B-OCD-004.
...
- Before ADR-B-OCD-004, multi-key failures can leave inconsistent test materialization.
...
Rejected because backend substitution already occurs through ADR-B-OCD-001 reader/writer handles. Concrete repository structs have one implementation and provide a smaller interface.
...
Deferred because no production state requires multi-version compatibility. ADR-B-OCD-006 introduces real canonical/schema version semantics.
...
Rejected because they are compact protocol scheduling state. Moving ordered lifecycle selection into MongoDB would expand ADR-B-OCD-002 beyond the body/repository boundary.

### Add repository transactions or manual rollback

Deferred to ADR-B-OCD-004. ADR-B-OCD-002 has no live multi-key writer and should not pre-design ExEx block atomicity.

## Verification

Run the repository test suites against both ADR-B-OCD-001 adapters.
...
ADR-B-OCD-002 does not change live node execution, so no hard fork or chain reset is required.
...
ADR-B-OCD-003 can define complete receipt-visible events for Tribute, Nod item, and Nod bucket mutations. Those events will carry enough typed data for ADR-B-OCD-004 ExEx projection to populate the repositories without re-executing domain business logic.

## Open questions and technical debt

- Replace temporary Postcard bodies with the canonical versioned codec governed by
  ADR-B-OCD-006, or prove the temporary format cannot enter durable state.
- Convert deliberately non-atomic repository writes into one planned batch before
  this ADR can be Accepted.
- Prove secondary indexes cannot retain orphan keys after update, delete, crash or replay.
- Reconcile body/EVM ownership field by field with ADR-C-TRB-001 and ADR-C-NOD-001; every
  duplicated mutable fact needs one authority.
- Add production-path pagination, malformed schema and concurrent-reader tests for all
  three record types.
