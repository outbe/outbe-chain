# ADR-B-OCD-009: Add provisional fixed sharding to the CKB SMT

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-16
- **Depends on:** ADR-B-OCD-008

## Context

ADR-B-OCD-008 implements the exact-parent authenticated CKB SMT, immutable candidate batches, CE-owned MDBX persistence, finalized-marker ordering, and the EVM-authoritative root seam. It deliberately remains an unsharded pre-activation reference stage. Its production constructors still use prebenchmark CE work/cache bounds, and its Criterion harness is a microbenchmark rather than activation evidence.

ADR-B-OCD-009 evaluates and introduces fixed power-of-two sharding behind the existing `AuthenticatedParentTree`/`CompressedTreeService` interface. It does not change ADR-B-OCD-006 body/leaf bytes, ADR-B-OCD-007 lifecycle or overlays, exact finalized-parent requirements, EVM root authority, candidate isolation, finalization ordering, or the rule that local tree failure never becomes a negative consensus vote.

ADR-B-OCD-009 is not deployed independently. ADR-B-OCD-010 places the same shard-set mechanism under collections and the Root Catalog using provisional pre-production `K = 16`. ADR-B-CAP-001 later benchmarks the complete ADR-B-OCD-001 through ADR-B-OCD-015 plus co-located off-chain-computation system and selects the production shard count. Neither the unsharded reference topology nor the ADR-B-OCD-009 intermediate topology consumes a `commitment_scheme_version`.

## Starting system

The implemented ADR-B-OCD-008 system has:
...
ADR-B-OCD-009 through ADR-B-OCD-015 use:
...
Production vectors also derive real Tribute, NodItem, and NodBucket `tree_key` values through ADR-B-OCD-008 and pin their shard indices. Cross-architecture tests must reproduce the same indices byte-for-byte.

### Shard-root aggregation

ADR-B-OCD-009 aggregates the fixed ordered shard-root vector with the concept's reserved CES1 tag:
...
Sealing starts from that verified parent vector, replaces only roots produced by touched shards, and recomputes the complete fixed top. With candidate `K <= 32`, the normative simple path performs exactly `K - 1` top-node hashes for a changed block rather than introducing a persistent top-node cache. A zero-touch block retains the already verified parent root and emits the ordinary ADR-B-OCD-008 identity candidate.

The ADR-B-OCD-009 intermediate EVM root is `shard_top_root`, not `R_sealed`. No temporary wrapper or temporary tag is introduced. ADR-B-OCD-010 reuses the same `TAG_TOP_NODE` construction and binds `commitment_scheme_version`, `collection_key`, `K`, and `shard_top_root` under `TAG_COLLECTION_ROOT` before committing collection roots through the Root Catalog.

#### Consequences and trade-offs

Benefits:

- the formula is identical to the shard-top portion of ADR-B-OCD-010 rather than a throwaway topology;
...
- the sharded root intentionally differs from ADR-B-OCD-008's unsharded root for the same logical leaves;
...
- the pre-activation ADR-B-OCD-009 `shard_top_root` does not independently encode `K`; the configured layout and local environment identity bind this test stage, while ADR-B-OCD-010 adds the explicit consensus hash binding before first deployment;
- `K = 1` is a deliberate special case with no `TAG_TOP_NODE` hash.

Rejected alternatives are a second CKB SMT over shard indices (unnecessary proof/update/storage machinery), a linear Poseidon fold (O(K) path proofs and weaker parallel structure), and a temporary `TAG_SHARD_SET_ROOT` wrapper that ADR-B-OCD-010 would immediately remove.

### Atomic multi-shard candidate batch

ADR-B-OCD-009 retains one immutable block candidate and one finalized commit boundary. The existing batch role becomes a typed shard-set envelope:
...
A zero-touch or all-net-no-op block carries equal parent/new vectors and an empty `changed_shards` map while retaining the ADR-B-OCD-008 identity candidate and marker progression. Complete vectors cost `2 * K * 32` bytes (2 KiB at `K = 32`) before small fixed framing, and that cost is included in `encoded_size` and candidate-cache accounting.

### MDBX sharded local codec

All shards remain in one CE-owned MDBX environment. ADR-B-OCD-009 advances `CE_MDBX_LOCAL_SCHEMA_VERSION` from 1 to 2 and uses new V2 tables; this is distinct from the unchanged EVM `0xEE0D.storage_schema_version = 2`. It never interprets ADR-B-OCD-008 unsharded keys as shard `0`:
...
- preserves ADR-B-OCD-008's one candidate, one atomic commit, one marker, and one ACK boundary;
...
- BE4 prefixes compose naturally with ADR-B-OCD-010's future `collection_key || shard_index` namespace;
...
- V2 tables and environment identity make ADR-B-OCD-008 MDBX disposable rather than reusable;
- ADR-B-OCD-010 will still extend the local namespace with collection identity before first deployment;
...
ADR-B-OCD-009 requires a sequential reference implementation. Mutations are grouped by `ShardIndex`, sorted by pinned CKB order, prepared against authenticated parent shard roots, and collected in ascending shard order before the coordinator constructs the one atomic batch.
...
ADR-B-OCD-009 does not prescribe a worker pool, concurrent-seal session model, fairness algorithm, reader formula, or production worker count before the sharded prototype and pinned Reth execution concurrency are measured. The candidate explicit-pool design, its benefits/costs, and its open concurrency questions are retained in ADR-B-CAP-001.

### CE work accounting boundary

ADR-B-OCD-009 preserves ADR-B-OCD-008's deterministic pre-write CE work admission and adds shard identity to the measurements. The preferred future decomposition is `base(K) + first-touched-shard + unique-key`; worker scheduling never changes consensus units, and no post-seal branch count may retroactively invalidate receipt-visible execution.

ADR-B-OCD-009 does not fix new coefficients, shard meter structures, or production limits. The full model, transaction/checkpoint semantics, alternatives, benefits, and costs are retained for measurement and closure in ADR-B-CAP-001.

### Candidate-cache boundary

ADR-B-OCD-009 batches report canonical encoded bytes and changed shard/branch/leaf counts, include full root vectors/prefixes, and retain explicit no-implicit-eviction/no-disk-spill semantics. These measurements are sufficient for the sharding comparison.

ADR-B-OCD-009 does not replace current cache configuration with a five-dimensional production envelope. The proposed per-candidate/total byte-and-record model, its startup relationship to CE capacity/candidate concurrency, and its benefits/costs move to ADR-B-CAP-001.

## Developer baseline: not activation evidence

The existing ADR-B-OCD-008 harness was run on 2026-07-16 with:
...
These numbers do not close ADR-B-OCD-009. The harness uses only 256 leaves, synthetic MDBX changes, and no complete execution/seal/finalization path.

## Verification benchmark

ADR-B-OCD-009 benchmarks `K_PROVISIONAL = 16` only enough to validate correctness and expose obvious regressions: deterministic insert/update/delete/mixed and all-in-one-shard cases, exact-parent proof/seal, aggregation, candidate bytes/records, finalized apply, restart, and optional sequential/parallel byte equality. Commands, fixture checksums, roots, and raw timings are retained, but they are not production capacity evidence.

ADR-B-CAP-001 owns the power-of-two candidate matrix, final hardware/full-path/off-chain-contention benchmark, `K_PRODUCTION`, CE/gas/resource limits, strict latency gate, and activation artifacts.

## Remaining closure

No numerical shard-count choice remains open for ADR-B-OCD-009 implementation: use 16. ADR-B-OCD-009 remains proposed until the `K = 16` sharded correctness/persistence path and tests are complete.

## Reset and version policy

ADR-B-OCD-009 performs no independent network activation or reset. Temporary unsharded materialization is disposable. ADR-B-OCD-010 completes the pre-production/testnet scheme-1 topology with `K_PROVISIONAL = 16`; ADR-B-CAP-001 later selects production `K` and requires a complete reset/rebuild if it changes before production. An in-place state-preserving K change is not allowed under the same scheme.

## Next step

Implement and verify the parameterized sharded path with active `K_PROVISIONAL = 16`, then continue with ADR-B-OCD-010 without waiting for production performance closure.

## Open questions and technical debt

- Shard count remains provisional until ADR-B-CAP-001 produces reproducible evidence on the
  minimum validator profile.
- Prove shard selection and root aggregation using independent golden vectors.
- Define atomic failure behavior for transactions spanning several shards.
- Bound parallel workers, candidate memory and MDBX transactions without introducing
  nondeterministic root or error order.
- Benchmark adversarial key skew rather than assuming uniform distribution.
