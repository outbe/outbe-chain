# ADR-B-OCD-004: Project finalized body events into MongoDB through Reth ExEx

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-15
- **Depends on:** ADR-B-OCD-003

> **ADR-B-OCD-006 integration note:** the coordinated reset replaces ADR-B-OCD-004's explicit-field event decoder and Postcard value construction with strict-canonical Protobuf payload verification. ADR-B-OCD-004's finalized-only source, full-block prepare, per-receipt atomicity, checkpoint-last rule, and crash replay remain authoritative. Because Mongo may contain partially applied receipt batches above its durable block checkpoint after a crash, ADR-B-OCD-006 ExEx validation does not compare the first event's `previousCommitment` with the current Mongo body; execution and independent ordered replay validate that pre-state, while ExEx validates the new leaf and same-identity event continuity within the block.

## Context

ADR-B-OCD-001 introduced backend-neutral Memory and MongoDB storage adapters. ADR-B-OCD-002 added typed Tribute and Nod repositories over six body/index namespaces. ADR-B-OCD-003 added complete `Stored` and identity-only `Deleted` events for every successful mutation of:
...
This ADR adds the first live receipt-to-Mongo materialization. It does not switch execution or business reads to MongoDB. That cutover and its stricter node-availability policy belong to ADR-B-OCD-005.
...
ADR-B-OCD-004 is a materialization stage only.
...
- The eight-second reconnect deadline, startup readiness gate, validator-participation gate, and whole-node shutdown policy are recorded in ADR-B-OCD-005, where MongoDB becomes an execution dependency.
...
A missing projector or MongoDB configuration is a startup configuration error and stops node startup. After a successful startup, ExEx projects asynchronously: consensus, execution, and existing EVM-backed reads do not wait for each MongoDB block checkpoint during ADR-B-OCD-004.
...
ADR-B-OCD-001 deliberately deferred cross-key transactions until a real consumer existed. ADR-B-OCD-004 adds that capability to `outbe-offchain-storage` rather than exposing MongoDB sessions to ExEx or domain repositories.
...
ADR-B-OCD-003 does not provide an event version, hash version, body commitment, or `body_hash`. ADR-B-OCD-004 therefore stores the emitter and exact event signature but does not invent `event_schema_version` or `body_hash` fields. Those concepts remain deferred to ADR-B-OCD-006.
...
A binary must explicitly support the stored local schema version. ADR-B-OCD-004 does not perform in-place schema migration. An incompatible database stalls the projector and requires a compatible snapshot or complete rebuild.
...
Filter only the six exact `(owning emitter address, event signature)` pairs defined by ADR-B-OCD-003:
...
ADR-B-OCD-003 intentionally has no version envelope. A later ABI change therefore requires coordinated hard-fork/binary rollout or a pre-production reset; ADR-B-OCD-004 does not invent runtime version negotiation.
...
Different receipts in a block commit sequentially and may become visible progressively. ADR-B-OCD-004 deliberately does not add block-wide MVCC, historical body versions, a block-wide read barrier, or one transaction spanning the full block.
...
Runtime does not read MongoDB in ADR-B-OCD-004, so progressive visibility cannot alter execution. ADR-B-OCD-005 gates execution on a complete parent checkpoint before making Mongo reads authoritative.
...
### Failure policy in ADR-B-OCD-004

ADR-B-OCD-004 distinguishes projection health from node health.
...
Because runtime reads remain EVM-backed in ADR-B-OCD-004, these failures do not stop the whole node. The ExEx task must remain alive in a stalled/degraded state rather than return unexpectedly through Reth's critical-task wrapper.

ADR-B-OCD-005 promotes MongoDB to an execution dependency and therefore strengthens these failures into readiness, participation, retry-deadline, and shutdown behavior.

### Snapshot restore

Snapshot creation, transfer, signing, backup, and restore are MongoDB operator responsibilities. ADR-B-OCD-004 does not add a custom `outbe-chain` snapshot import/export command.
...
- The storage facade and conformance suite become wider than ADR-B-OCD-001's original per-key contract.
...
Rejected for ADR-B-OCD-004. The accepted materialization is finalized-only and therefore does not require a MongoDB reorg journal. ADR-B-OCD-005 coordinates execution with the complete finalized-parent projection before Mongo becomes authoritative.
...
Rejected because it would repeatedly suspend user reads while a complete block is applied. ADR-B-OCD-004 accepts cross-receipt eventual visibility and uses a final checkpoint as the durable completeness marker.
...
Verify in ADR-B-OCD-004 specifically that:
...
ADR-B-OCD-004 changes only node-local derived storage and node wiring. It does not change block execution, receipts, or consensus state beyond the already activated ADR-B-OCD-003 event contract.

MongoDB may be dropped and rebuilt from retained compatible receipts. If the ADR-B-OCD-003 event ABI changes simultaneously, use its coordinated hard fork or complete testnet reset.

Pre-production incompatible local schema changes may use a complete MongoDB rebuild rather than an in-place migration.

## Next unlocked step

ADR-B-OCD-005 can switch body-dependent execution and query paths to the populated repositories. At that point MongoDB becomes an explicit testnet execution dependency, and projection readiness must gate validator participation and whole-node availability.

## Open questions and technical debt

- **Critical:** retain the current sticky projection fatal behavior and prove every
  projector exit removes node/validator readiness through ADR-B-SUP-001 supervision.
- Enforce one writer using a durable lease/fencing epoch, especially when validators
  share one Mongo deployment.
- Prove block preparation is side-effect free and one Mongo transaction commits all
  documents, indexes and checkpoint or none.
- Test duplicate, gap, restart and checkpoint-ahead/behind behavior against the actual
  Reth finality notification source.
- Snapshot restore and historical receipt availability remain open under ADR-B-OCD-014 and
  ADR-B-OCD-015.
