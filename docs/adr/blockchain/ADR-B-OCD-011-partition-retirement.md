# ADR-B-OCD-011: Retire completed Tribute partitions through one Root Catalog delete

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-17
- **Depends on:** ADR-B-OCD-010

## Context

ADR-B-OCD-010 gives every Tribute Worldwide Day (WWD) an independent authenticated collection whose non-zero collection root is stored as one Root Catalog leaf. Ordinary entity deletion deliberately retains an empty collection leaf, while no operation can remove a finished collection or reclaim its finalized CE MDBX namespaces.
...
At begin block, both retirement structures must be empty. `end_block` consumes them before clearing every dynamic record and list entry. A dirty begin state, duplicate/unreachable touched record, malformed marker, cleanup failure, or non-empty post-condition is fatal block execution, matching ADR-B-OCD-007 overlay rules.

Adding this journaled structure advances the `0xEE0D` EVM storage schema at the ADR-B-OCD-011 activation fork. The additive migration requires the ADR-B-OCD-010 entity overlay and new retirement state to be empty at the boundary, initializes the new fields to empty, and updates slot 0 through the normal hard-fork migration path. It does not add dual reads, dual writes, or a compatibility mode.
...
ADR-B-OCD-011 does not add WWD filtering to point, owner, day, or pagination paths. Production read semantics are reconsidered with the later off-chain-computation apply boundary. If that boundary guarantees retirement after the last compressed-body read, no read-aware overlay is needed; otherwise a later ADR may strengthen it with measured implementation evidence.
...
ADR-B-OCD-010's collection change envelope becomes an explicit canonical variant, conceptually:
...
The canonical batch visitor owns the operation discriminant, ordering, encoded size, and checksum; there is no retirement-specific parallel codec. The exact bytes are pinned with ADR-B-OCD-011 golden vectors after the mandatory seam review against the implemented ADR-B-OCD-010 envelope.
...
MDBX readers observe either the complete old materialization or the complete retired materialization. A committed Catalog non-membership can never coexist with orphan shard records/root entries, preserving ADR-B-OCD-010's corruption invariant. Transaction abort, process crash, uncertain commit, and redelivery follow ADR-B-OCD-008 through ADR-B-OCD-010 marker/idempotency recovery; no partially reclaimed namespace is accepted.

Synchronous prefix reclamation can increase finalized-apply latency for a large WWD. The testnet accepts that cost. ADR-B-CAP-001 measures the completed path and owns any later bounded garbage-collection/generation design; ADR-B-OCD-011 does not add an asynchronous local cleanup protocol speculatively.
...
Worker topology, computation proofs/attestation, concurrency, caches, resource sharing, and production timing are outside ADR-B-OCD-011. They require their own seam review and measured closure; retirement must not embed assumptions about a provisional worker implementation.
...
- `R_sealed` remains EVM-state-only until ADR-B-OCD-012; proof RPC, recovery audit,
  and portable snapshots remain ADR-B-OCD-013 through ADR-B-OCD-015 work.
...
Exercise commit-before-ACK redelivery, restart after uncertain commit, multiple retired WWDs over time, and absence of orphan namespace records. Benchmark namespace deletion separately and in the full finalized path for later ADR-B-CAP-001 closure.

## Reset and activation policy

ADR-B-OCD-011 is an additive testnet hard fork after the ADR-B-OCD-010 CES1 reset. It changes no body commitment, EntityId36, collection key, tree key, shard count, collection root, Catalog key, or `R_sealed` formula. Existing CE MDBX V3 data remains structurally compatible.
...
Before implementation, ADR-B-OCD-011 must be reviewed against the actual ADR-B-OCD-010 candidate envelope, canonical visitor, storage layout, `AuthenticatedCatalogView`, and finalized apply code. Concrete field placement and operation bytes are pinned only through that seam review and golden vectors, without changing the decisions above.

## Next unlocked step

ADR-B-OCD-012 carries the stable execution-computed `R_sealed` directly in block header artifacts while retaining EVM `0xEE0D.slot1` as execution authority.

## Open questions and technical debt

- Keep Lysis completion and authorization in ADR-C-LYS-001; this layer consumes a typed
  retirement command and must not duplicate business policy.
- **Critical:** prove no raw storage, factory or lifecycle bypass permits a same-block
  mutation after retirement has been requested.
- Define physical namespace reclamation relative to finality, crash recovery and proof
  retention.
- Make catalog delete, event, projection cleanup and permanent non-reuse atomic or
  explicitly reconciled.
- Add failure-injection and restart tests for every retirement stage.
