# ADR-B-OCD-006: Commit to canonical Tribute and Nod bodies and verify MongoDB reads

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-15
- **Depends on:** ADR-B-OCD-005

## Context

ADR-B-OCD-005 makes MongoDB the source of complete Tribute, Nod item, and Nod bucket bodies used by execution. Projection provenance and strict repository decoding detect malformed storage, but a well-formed altered, stale, or wrong body is not yet authenticated against consensus state.

ADR-B-OCD-006 adds deterministic body commitments and verifies actual MongoDB reads before domain logic may use them. It does not add an SMT, global root, proof service, authenticated secondary-index completeness, or off-chain computation.
...
ADR-B-OCD-006 uses these immutable CES1 tags from the compressed-entity design:
...
TAG_KEY          = CES1_TAG_BASE + 5  // ADR-B-OCD-008 CKB SMT key
TAG_BODY         = CES1_TAG_BASE + 6
TAG_LEAF         = CES1_TAG_BASE + 7
TAG_SMT_BASE     = CES1_TAG_BASE + 8  // ADR-B-OCD-008 CKB MergeValue codec
TAG_SMT_NORMAL   = CES1_TAG_BASE + 9
TAG_SMT_ZERO     = CES1_TAG_BASE + 10
CES1_TAG_BASE     = 0x4345533100000000
```

ADR-B-OCD-008 uses `TAG_KEY` and the three CKB MergeValue tags without changing ADR-B-OCD-006 body/leaf commitments. `PBytes(object_tag, bytes)` is the only arbitrary-byte conversion:
...
ADR-B-OCD-006 does not build an SMT. The current commitment authority is three distinct typed EVM mappings:
...
Create writes a non-zero leaf, update replaces it, and delete clears it to zero in the same EVM journal/revert scope as the domain mutation and body event. ADR-B-OCD-007 centralizes those transition and same-block overlay rules. ADR-B-OCD-008 later replaces the direct mappings behind that lifecycle with an SMT while preserving the leaf formula.
...
ADR-B-OCD-006 authenticates point bodies. A direct point lookup first reads the EVM commitment; zero is canonical absence, while non-zero requires an available matching MongoDB body.
...
ADR-B-OCD-006 does not authenticate secondary-index completeness, omitted IDs, or page continuation. That limitation already belongs to the ADR-B-OCD-005 testnet trust model and is not solved or redesigned here. An authenticated entity SMT alone will not prove a secondary list complete; any production completeness claim requires a separate authenticated-index design.

### Storage ownership

ADR-B-OCD-006 defines three logically distinct typed commitment mappings as a focused implementation/test backend. ADR-B-OCD-007 finalizes their temporary physical owner, and ADR-B-OCD-008 replaces them before the first combined deployment: `outbe-compressed-entities` at system state address `0x000000000000000000000000000000000000EE0D` owns the Tribute, Nod item, and Nod bucket namespaces together with the shared journaled overlay.
...
ADR-B-OCD-008 replaces the direct mapping backend inside that same module, ADR-B-OCD-009 shards it, and ADR-B-OCD-010 completes the first deployed Root Catalog topology. The single coordinated testnet reset then removes legacy body/index fields and initializes only the final system without a live-state migration.
...
- non-zero commitment plus backend `Unavailable` enters the ADR-B-OCD-005 shared `MongoUnavailable` recovery path and locally aborts/abstains without a `false` vote;
...
Only technical backend unavailability receives the ADR-B-OCD-005 eight-second recovery window. Deterministic corruption, unsupported protocol data, and projector/event invariant failures initiate immediate structured graceful shutdown. These local failures do not make the node cast a negative consensus vote.
...
Rejected because Postcard was explicitly temporary in ADR-B-OCD-002 and ties consensus bytes to Rust/Serde layout rather than a published append-only schema contract.
...
Rejected because the accepted future tree uses Poseidon-BN254. Using the CES1 leaf now avoids replacing every current commitment when ADR-B-OCD-008 introduces the SMT.
...
Rejected because `WWD_BE4 || Poseidon_BE28` reduces generic collision security from approximately 127 bits for full BN254 output to approximately 112 bits. The custom bytes ABI accepts `WWD_BE4 || Poseidon_BE32`, so ADR-B-OCD-006 preserves the full digest instead of spending security margin to retain `uint256` compatibility.
...
- every CES1 tag used by ADR-B-OCD-006;
...
ADR-B-OCD-006 changes canonical body bytes, event signatures, EVM layouts, and execution-visible verification. Implement it on the same branch as ADR-B-OCD-005 through ADR-B-OCD-010, then use the single complete coordinated testnet reset only after ADR-B-OCD-010 and combined verification.
...
ADR-B-OCD-007 can place the three typed commitment mappings behind one generic mint/update/delete lifecycle and add the permanent journaled body overlay for deterministic same-block reads and rollback.

## Open questions and technical debt

- Publish one canonical manifest for identity bytes, Protobuf rules, Poseidon parameters,
  domain tags, commitment scheme and independent golden vectors.
- Enforce append-only schema evolution mechanically; prose does not prevent field-number
  or wire-type reuse.
- Prove domain/schema/full-body binding without ambiguous field reduction or collision.
- Audit all read routes for verify-before-use and separate corruption from absence and
  transient backend failure.
- Define activation and migration for future versions rather than relying permanently
  on coordinated testnet resets.
