# ADR-B-OCD-003: Publish complete off-chain records in receipt events

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-15
- **Depends on:** ADR-B-OCD-002

> **Supersession note:** ADR-B-OCD-006 replaces the explicit-field Stored/Delete ABI defined here during the single coordinated ADR-B-OCD-003 through ADR-B-OCD-010 first CES1 testnet reset. ADR-B-OCD-003 remains authoritative for receipt visibility, complete mutation publication, owning emitters, journal/revert atomicity, system-receipt inclusion, and deterministic log ordering. The active ADR-B-OCD-006 wire form publishes strict-canonical Protobuf payloads, 36-byte IDs, schema/commitment versions, and previous/new commitments instead of duplicating every body field as Solidity arguments.

## Context

ADR-B-OCD-001 introduced the domain-neutral off-chain storage facade. ADR-B-OCD-002 added typed Tribute and Nod repositories for:
...
The next step is to publish every complete off-chain record mutation in normal transaction receipts. ADR-B-OCD-004 can then project those receipts into MongoDB without re-executing Tribute or Nod business logic.
...
The state-owning domain address is the authoritative emitter for its projection events. ExEx will use both event signature and emitter address when ADR-B-OCD-004 defines its filter.
...
`TributeBodyDeleted` contains only `token_id`. ADR-B-OCD-004 will load the current projected body before repository deletion so owner/day indexes remain hidden behind `TributeRepositoryWriter::delete(token_id)`.
...
For Nod issue and mining, item projection precedes bucket projection. This order is part of the receipt contract. ADR-B-OCD-004 will apply all projection mutations from one successful EVM receipt atomically while allowing different receipts in the same block to materialize sequentially.
...
4. delegate Postcard encoding and index maintenance to the ADR-B-OCD-002 repository writer.
...
Canonical schema/hash versions are introduced with commitments in ADR-B-OCD-006. A domain registry is introduced only when a real later domain or runtime version requires it.
...
The receipts now contain enough information for ADR-B-OCD-004 to rebuild all six repository collections without re-executing domain business logic.
...
- Repository body/index writes are still non-atomic until ADR-B-OCD-004.
...
Rejected because it would make the Rust repository encoding part of the public EVM receipt ABI. Explicit fields are transparent to standard tooling, and ADR-B-OCD-004 can construct the typed record before repository encoding.
...
Rejected because ADR-B-OCD-002 writers own index cleanup and already load the current primary body by identity. Delete events remain small and identity-only.
...
Projection events change receipts and therefore block results. Activate ADR-B-OCD-003 through a coordinated hard fork or complete testnet restart.

No historical event migration or dual decoder is required. MongoDB may be empty or deleted because ADR-B-OCD-004 has not yet made it a chain-derived materialization.
...
ADR-B-OCD-004 can install a Reth ExEx that:
...
- applies Stored/Deleted events through the ADR-B-OCD-002 typed writers;
...
- Reconcile every event signature, indexed field and discriminant against the current
  generated ABI; examples in this migrated proposal are not an activation manifest.
- Prove every production mutation route emits exactly one reconstructible projection
  event, including lifecycle hooks, factories, deletes and same-block replacements.
- Specify deterministic order for repeated changes to one entity in a transaction/block.
- Version event semantics before any incompatible body/schema change.
- Add negative tests for wrong emitters, malformed logs, reverted calls, duplicates and
