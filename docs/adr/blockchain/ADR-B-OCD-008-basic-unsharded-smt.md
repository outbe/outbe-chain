# ADR-B-OCD-008: Replace direct commitment mappings with a vendored unsharded CKB SMT

- **Status:** Proposed; migrated design, current implementation evidence requires reconciliation
- **Date:** 2026-07-15
- **Depends on:** ADR-B-OCD-007

## Context

ADR-B-OCD-007 places Tribute, Nod item, and Nod bucket behind one compressed-entity lifecycle seam. It keeps three direct EVM commitment mappings as the current-state authority and records one reversible, journaled final body mutation set for deterministic end-block cleanup and later sealing.

Direct mappings authenticate point reads but do not produce one compressed-entity root or portable inclusion/non-inclusion evidence. ADR-B-OCD-008 replaces only that backend with one unsharded sparse Merkle tree while retaining ADR-B-OCD-006 bodies/events and ADR-B-OCD-007's domain interface, body/index overlay, transition semantics, rollback, and same-block reads.

ADR-B-OCD-008 adopts the ready CKB `sparse-merkle-tree` engine and finalized persistence mechanics selected by `compressed_entities_concept_v6_proposed_10-07-2026.md`. It does not design a new tree engine. The sequential ADRs remain authoritative where the older all-at-once concept differs: ADR-B-OCD-006 uses EntityId36 plus strict-canonical Protobuf; ADR-B-OCD-007 uses `0xEE0D`, domain-address mutation events, and body/index overlays; ADR-B-OCD-008 is deliberately one unsharded tree without the concept's domain registry, collection shards, Root Catalog, retirement, or header carrier. Those later capabilities remain separately staged.

ADR-B-OCD-008 does not add sharding, collection roots, partition retirement, a header artifact, public proof RPC, portable snapshots, or production capacity claims.
...
- zero as canonical absence and non-zero ADR-B-OCD-006 leaves as current values;
...
The engine remains a 256-level H256 sparse tree. ADR-B-OCD-008 does not replace it with a custom 254-level implementation. The byte bridge is exact and has no byte reversal:
...
The ADR-B-OCD-006 non-zero body commitment is stored verbatim as the CKB leaf value:
...
All three logical collections share this stage's one tree, so the key binds the fork-fixed typed collection outside EntityId36 and the ADR-B-OCD-006 leaf:
...
Collection is included because equal EntityId36/`identity_f` values in different typed collections must occupy different positions. Collection is selected by ADR-B-OCD-007's closed typed interface and is never caller-controlled.

`tree_key_f` is a canonical Poseidon-BN254 output, never arbitrary bytes reduced modulo the field. Key zero is a valid position; zero is reserved only as the absent leaf value. This exact key recipe is normative for the unsharded reference stage. ADR-B-OCD-010 may supersede fixed `collection_id` with its final collection-key derivation before first activation without consuming a version. After scheme 1 is deployed, changing its tag/input order, collection derivation, H256 encoding, or CKB path semantics is a commitment-scheme change.

ADR-B-OCD-006 through ADR-B-OCD-010 have no intermediate testnet activation. This unsharded tree is therefore a reference/benchmark milestone using the final CES1 hash/leaf primitives, not a separately deployed commitment scheme. ADR-B-OCD-009 and ADR-B-OCD-010 may change topology without consuming versions before launch. The first deployed complete collection/Root Catalog construction after ADR-B-OCD-010 is `commitment_scheme_version = 1`; `tree_format/vendor_revision` remains local MDBX metadata. Only a semantic change after that activation increments the network scheme version.
...
Structural emptiness is CKB `ZERO`: the empty unsharded tree root is zero. If any Poseidon result computed for non-empty content evaluates to zero, sealing fails deterministically. Body leaf zero is already rejected by ADR-B-OCD-006.
...
ADR-B-OCD-008 replaces temporary schema v1 on the development branch with the root-backed layout below, but does not activate it on testnet before ADR-B-OCD-009 and ADR-B-OCD-010:
...
The former Tribute direct-map base at slot 1 becomes the root slot. Former mapping-base slots 2–3 remain empty/reserved in v2 rather than shifting the permanent ADR-B-OCD-007 overlay or prematurely assigning future topology state. No direct-map entry is migrated, read, or retained through fallback; the complete reset guarantees their derived storage keys do not exist in the new chain.

During execution of block `B`, slot 1 remains `R(B-1)` until compressed-entity end-block sealing. Same-block current values come from ADR-B-OCD-007's overlay over the exact parent SMT. Successful seal writes `R(B)` in the same buffered post-block change set that clears overlay storage.

The empty unsharded CKB reference root is ZERO. ADR-B-OCD-008 harness/replay initialization uses a matching height-0 marker, but this state is not a network genesis. The first deployed empty root is ADR-B-OCD-010's derived scheme-1 `R_sealed` over the final shard/collection/catalog topology and may be non-zero; final slot 1 and CE marker must match that derivation exactly. The root is never an independent operator value.

### Journaled execution and staged sealing

The persistent CKB tree is not mutated transaction by transaction and is not part of REVM checkpoint/rollback. ADR-B-OCD-007's journaled EVM overlay remains the only block mutation accumulator. Ordered operation history remains in canonical receipt events; sealing consumes one final value per touched key.

ADR-B-OCD-008 makes the lifecycle output type explicit across the workspace:
...
1. reads and validates every unique ADR-B-OCD-007 body identity record and final pending word;
...
ADR-B-OCD-005's testnet execution profile permits block execution only over an exact finalized parent whose Mongo projection is ready. ADR-B-OCD-008 adds the matching tree prerequisite: that same parent must already be committed in CE MDBX with equal height/hash/root. A non-finalized staged batch is never an execution parent in this stage.
...
ADR-B-OCD-008 includes durable finalized SMT storage from its first implementation. It does not defer baseline persistence to the legacy reserved ADR-014 stage.
...
ADR-B-OCD-008 preserves ADR-B-OCD-007's decision not to add a separate mutation-count cap and separates two accounting dimensions.

Ordinary EVM transaction gas covers synchronous call work: body hashing/encoding, event bytes, overlay/index storage, point/list reads, and ADR-B-OCD-007's first-touch cleanup/seal surcharge. It is charged by the transaction, appears in receipts/cumulative header `gas_used`, follows normal revert/OOG rules, and is pinned to the active EVM schedule.
...
ADR-B-OCD-007 lifecycle/business reverts remain ordinary receipt outcomes and leave no generic mutation/event state. ADR-B-OCD-008 adds:
...
Mongo projection readiness and tree readiness are evaluated as one execution prerequisite against the same required parent and remaining request/view budget, preferably concurrently. They do not consume two sequential timeout windows. Mongo retains ADR-B-OCD-005's supervisor/recovery deadline; a longer tree rebuild leaves the node non-participating and cannot accidentally inherit or extend that Mongo timeout.
...
- same-block body/index behavior remains ADR-B-OCD-007 overlay-first;
...
- Portable snapshots, peer recovery, authenticated secondary-index completeness, and production availability guarantees remain outside ADR-B-OCD-008.
...
- The ADR-B-OCD-007 lifecycle/domain interface remains unchanged when the backend moves from direct maps to SMT.
...
Rejected because the selected CKB store and finalized proof/readiness path need one atomic node/marker owner from the first SMT stage. The former legacy reserved ADR-014 persistence stage is folded into ADR-B-OCD-008.
...
ADR-B-OCD-008 is not separately deployed and performs no testnet reset. It is the unsharded reference/benchmark implementation on the ADR-B-OCD-005 through ADR-B-OCD-010 branch; ADR-B-OCD-009 and ADR-B-OCD-010 replace its root topology before the first CES1 network starts. There is no live migration, dual-write/read, direct-map fallback, or intermediate binary deployment.

After ADR-B-OCD-010 and the combined suite, one coordinated reset allocates final marker-preserved `0xEE0D`. Unless ADR-B-OCD-009 and ADR-B-OCD-010 require an explicitly documented EVM layout change, root-backed schema version 2 remains: slot 1 carries the final derived empty `R_sealed`, slots 2–3 are reserved, and slots 4–10 are empty overlay storage. CE MDBX initializes the final scheme-1 sharded collection/Root Catalog marker at height 0; no unsharded MDBX state is imported. MongoDB begins empty under the same reset.
...
ADR-B-OCD-009 benchmarks and introduces fixed power-of-two sharding over the same pinned CKB/Poseidon/store interfaces without an intermediate deployment or version bump. It preserves ADR-B-OCD-006 leaf bytes/formula, ADR-B-OCD-007 lifecycle/overlay semantics, exact finalized persistence ordering, and correct worst-case behavior when all mutations concentrate in one shard; ADR-B-OCD-010 then completes the first activated scheme-1 topology.

## Open questions and technical debt

- Bind the actually active topology and vendor revision into genesis/chain identity;
  this unsharded stage is not itself a production profile.
- **Critical:** prove finality acknowledgement cannot precede durable CE MDBX root apply.
- Audit speculative candidate retention, fork eviction and restart recovery for stale or
  unbounded branches.
- Replace string tree/vendor identity with canonical versioned identifiers and
  independent codec/root vectors.
- Close work, memory, map-size, disk-growth and compaction limits under ADR-B-CAP-001.
