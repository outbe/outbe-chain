...
The node derives its embedded OCOMP FullNode profile from genesis and its data
directory; there is no separate follower process or `--ocomp.*` control profile.
Run only the normal external SnapshotExporter and Worker services generated by
the launch bundle. The embedded Supervisor performs Lysis orchestration and
durably publishes the exact local result, but FullNode policy never opens the
validator attestation or vote-submission path.
...
`ocomp-registration-v1.ocb1`. A validator startup without the registered OCOMP
key is rejected. A FullNode needs no OCOMP signing key or registration; its
embedded runtime is initialized from genesis and the node data directory.
...
There are no node `--ocomp.*` flags. The node loads the protocol bundle from
genesis, resolves the OCOMP domain beside its data directory and owns the
embedded Worker endpoint at `consensus listen port + 1`. Start the external
SnapshotExporter and Worker with the matching generated launch bundle.
...
The node process owns the embedded Supervisor, its Worker registration endpoint
and the durable payout/sign-once journals. SnapshotExporter and Worker are the
only external OCOMP processes; both connect to the node-owned endpoint and use
the same protocol-bundle identity. OCOMP does not require fixed service
usernames. The OCOMP key file and journals under the chain data directory must
survive restarts. An exact retry returns the byte-identical stored signature; a
changed job binding is refused as equivocation.
...
- **FullNode receiving validator role** — stop the FullNode and restart the same
  ...
  consensus/EVM keys, OCOMP key and public DKG artifacts. Once the node-owned
  OCOMP endpoint is ready, start the external SnapshotExporter and Worker from
  the matching generated bundle. Do not repeat TEE join or replace any NodeHost
  artifact. The ordinary `registerValidator`, stake and `confirm-ready` path
  leads to PENDING; the next certified reshare makes it ACTIVE and its address
  appears automatically in snapshots for new OCOMP jobs.
