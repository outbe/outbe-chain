...
- The validator controls its EOA, BLS key, OCOMP result-signing key and valid
  optional versioned P2P address. P2P defaults to unset; when present, its version
  and payload decode atomically.
...
| 3 | node/operator | sync to finalized head and call `confirmValidatorReady(registration)` with the chain/genesis/validator-identity-bound OCOMP PoP | stored canonical registration, reverse key reservation, `Joining` (`PENDING`) and set-change signal |
| 4 | consensus/DKG | freeze canonical reshare target and complete ceremony | validated DKG artifact |
| 5 | ValidatorSet boundary | atomically snapshot committees plus their separate OCOMP key extensions and activate share | `Active` (`ACTIVE`), share, set hash, historical OCOMP binding |
...
Inactive re-registration and cleanup require the same cooldown, so cleanup cannot
bypass it. Byte-identical OCOMP registration replay is accepted; key epoch 1 does
not permit replacing the pinned OCOMP public key. Re-entry and BLS-identity
replacement clear readiness, and a BLS change requires a refreshed identity-bound
proof for the same pinned OCOMP key before another DKG can include the validator. DKG
