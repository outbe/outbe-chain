...
| PFS-001-10 | zk-enabled L2 operator without signature | sender registered in L2Registry by an approved validator proposal with `zk_enabled=true`; open day and valid canonical offer | submit offer with empty `zkMerkleRoot`/`signature`; then approve a `setZkEnabled` proposal and resubmit | first offer reverts with zero supply/projection change; after governed disablement an unsigned offer succeeds | `@pfs-001-10` live-node |
