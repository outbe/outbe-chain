...
| 3 | consensus/node | finalize request block, derive exact `JobId`/finality proof, bind it to its `InputLeaseId` and historical ValidatorSet snapshot in the multi-job registry, wait four additional blocks, then atomically install `VOTING_OPEN(open_height = finality_recorded_height + 4, deadline_height = open_height + job.capacity_profile.result_deadline_blocks)` and its deadline index | finalized Job entry, three snapshot bindings, `N`, quorum, referenced input lease and genesis-installed compute-vote window (production default 1800) | all nodes report the same binding/open/deadline; a retained predecessor and retry coexist; vote before finality or `open_height` rejects in OCOMP state; adversarial proof/overflow vectors reject | POC-04, POC-18 |
| 4 | supervisor | scan finalized `OffchainJobRequested` events as locators, then read the typed `OcompJobRecordV1` from local finalized RPC and bind it to the canonical request block; do not reconstruct finality or historical ValidatorSet proofs in the Supervisor; `VotingAuthority` exposes only `VotingOpen` inside the response window | local `DISCOVERED` journal containing the exact finalized `JobIntent`/`JobId`/request identity | dropping the request subscription still discovers exactly one job; event/state, request-root, chain/genesis/fork/bundle or window mismatch rejects; restart rereads the record at the exact finalized-head hash and rejects any journal mismatch; the production Supervisor issues no `eth_getProof` call | POC-05 |
| 5 | node/exporter | open the retained Tribute partition; full-fold CE; ask the local node's purpose-bound RPC to build exact request-state Fidelity/Oracle openings in consecutive owner batches; deterministically bisect a response above the bundle-pinned control cap; independently verify returned raw openings against the durable finalized `JobIntent`; publish manifest/chunks; `InputReplay` permits both `VotingOpen` and `Completed`, so a FullNode can finish and verify after the deadline or restart | finalized pin, `InputManifestV1`, CAS objects | every domain independently reconstructs exact root/count/nominal/opening commitments; RPC resolves the authoritative record and historical state locally; the vote path alone owns the deadline; `AwaitingFinality`, `Expired`, `Conflicted` and `Canceled` never substitute as replay authority; typed oversize rejection preserves owner order/completeness; one-owner oversize abstains; Mongo/CAS mutation rejects | POC-06, POC-09, POC-20 |
...
| 12 | remaining pinned validators/consensus | keep the quorum-complete job discoverable/exported/signable across restart, accept remaining full-result system votes until the exclusive job-pinned deadline, close accountability, record every missing pinned participant, and jail only one whose current status is still `ACTIVE` | dynamic matching/minority/missing/equivocation bitmaps separate from terminal result, durable live pin through deadline, plus replay-idempotent JobId/participant evidence | early quorum does not hide work from a remaining validator; healthy run records full participation; missing current-`ACTIVE` voters become `JAILED`; missing non-ACTIVE participants remain unchanged; timely minority voters are present; an authentic carrier executed after closure gets a `status = 0` deadline receipt without changing vote state or aborting the block; terminal result remains unchanged | POC-10, POC-18, POC-19 |
