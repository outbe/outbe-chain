...
| 12 | remaining pinned validators/consensus | accept remaining full-result system votes until the exclusive 1800-block deadline, close accountability, and jail every missing pinned participant exactly once | dynamic matching/minority/missing/equivocation bitmaps separate from terminal result plus replay-idempotent JobId/participant jail evidence | healthy run records full participation; absent voters become `JAILED`; timely minority voters are present; terminal result remains unchanged | POC-10, POC-18, POC-19 |
