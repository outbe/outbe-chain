...
| 3 | consensus/node | finalize request block, derive exact `JobId`/finality proof, bind it to its `InputLeaseId` and historical ValidatorSet snapshot in the multi-job registry, wait four additional blocks, then atomically install `VOTING_OPEN(open_height = finality_recorded_height + 4, deadline_height)` and its deadline index | finalized Job entry, three snapshot bindings, `N`, quorum, referenced input lease and canonical open-window state | all nodes report the same binding/open/deadline; a retained predecessor and retry coexist; vote before finality or `open_height` rejects in OCOMP state; adversarial proof/overflow vectors reject | POC-04, POC-18 |
...
| 12 | remaining pinned validators/consensus | accept remaining full-result votes until the exclusive deadline and then close accountability | dynamic matching/minority/missing/equivocation bitmaps separate from terminal result | healthy run records full participation; a tolerated unavailable domain leaves its missing bit without changing the completed terminal result | POC-10, POC-18, POC-19 |
