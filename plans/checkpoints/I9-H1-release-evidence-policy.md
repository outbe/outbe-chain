...
the production enclave-resident verifier. A failed Platform admission receives
...
- exact chain/genesis/active-policy binding for the production release;
- retained canonical evidence from the public Begin/Chunk/Finish verifier.

The current release evidence harness still uses its explicit test-only chain ID,
synthetic genesis hash and measurement-derived initial policy. That evidence is
useful for hardware-path qualification but cannot authorize a production
release. Before H1 can pass, the harness must consume the canonical production
chain/genesis and exact active policy bytes, and the finalizer must verify their
hashes against the evidence. Until then the protected workflow is ineligible
