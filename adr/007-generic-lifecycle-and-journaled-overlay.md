...
ADR-007 introduces one generic mutation lifecycle and the permanent block-scoped body overlay used by the later SMT stages. It does not build an SMT, global root, proof service, sharding, or partition retirement.
...
`stored_body` is the exact strict-canonical ADR-006 `StoredBody` envelope containing per-body schema version and canonical payload. A commitment-only overlay is insufficient because same-block domain reads need the complete new semantic body before MongoDB can project it.
...
mint/update:
  ...
  emit the ADR-006 typed Stored event
...
delete:
  ...
  append the identity on first touch only
  emit the ADR-006 typed Delete event
```

In ADR-007 storage schema v1, mutation-map writes, overlay writes, touched-list changes, domain writes, and event emission use one EVM journal/revert scope. ADR-008 schema v2 explicitly supersedes the direct-map portions of this pseudocode: mint/update/delete write only journaled overlay/index/event state during transactions, slots 1–3 are never mutated per entity, and the sole persistent root changes once in end-block sealing. Existing REVM checkpoints therefore revert nested-call failures, failed transactions, out-of-gas execution, and atomic system-handler failures without a second process-memory undo journal.
...
Here `p` is the BN254 scalar-field modulus. Every valid ADR-006 leaf is canonical, non-zero, and below `p`, so it cannot collide with `Deleted`. An invalid word is fatal rather than normalized.
...
After all receipt-producing system and user transactions and after the last body mutation, but before final state-root calculation, the executor invokes the explicit ADR-007 lifecycle cleanup.
...
This ADR-007 charge is ordinary EVM transaction gas: it contributes to receipt cumulative gas and header `gas_used`. Gas is deducted before the corresponding mutation/event writes. Insufficient gas reverts without committed module state or logs. End-block cleanup issues no refund and credits no arbitrary transaction. Reverted transactions consume work gas under ordinary EVM rules while their journaled overlay/reservation writes disappear. ADR-008 separately adds non-header CE work units for deferred tree/persistence liveness; those units never replace or alter this EVM gas accounting.
...
The commitment mappings remain the ADR-006 backend in this stage. ADR-008 replaces that backend with an SMT while retaining overlay-first reads and the generic lifecycle interface.

### Journaled query-index deltas

A point-body overlay alone is insufficient: MongoDB owner/day/global indexes describe the finalized parent and cannot include a body minted earlier in the current block or remove one deleted earlier in the block. ADR-007 therefore overlays the four active query surfaces:
...
Secondary-index completeness of the finalized-parent Mongo input remains the explicit ADR-005/006 testnet limitation. The delta overlay makes same-block changes correct relative to that parent; it does not prove the parent list complete.
...
The repository's existing page bound remains authoritative. ADR-007 adds no second arbitrary limit. Loop work is bounded by the requested page, matching same-block removals, and the block-gas-bounded touched list.
...
- under ADR-007 schema v1 immediately updates the direct commitment mapping and final pending state; under ADR-008 schema v2 updates only final pending state;
- emits its own canonical ADR-006 mutation event in execution order;
...
No MongoDB client, repository adapter, `StorageHandle` global, background task, dynamic plugin registry, or SMT implementation lives at this seam. ADR-008 deepens the same module internally by replacing direct-map fallback/sealing without changing domain callers.
...
`TributeBody`, `NodItemBody`, and `NodBucketBody` are the generated semantic Protobuf payload types fixed by ADR-006. Domain runtime converts its semantic record into the matching typed payload before calling the module. The `BodyInput` variant determines collection namespace, active codec, commitment mapping, event signature, and owning emitter; the complete ID is read from the body rather than supplied a second time.
...
ADR-005 `RuntimeBodyReaders` is the production adapter over the typed Mongo repositories. The test adapter uses Memory repositories. MongoDB/BSON/session types do not cross the seam.
...
A returned parent body is verified against the current direct-map leaf exactly as in ADR-006. `None` from the parent while the leaf is non-zero is `CommittedBodyMissing`, not absence. The parent adapter is never called for Set, Deleted, or mapping-zero identities.
...
Local parent-body outcomes retain ADR-005/006 classification:
...
Fatal/local-unavailable outcomes never become a `false` consensus vote. Proposer forfeits or verifier withholds under the ADR-005 policy; deterministic user/domain reverts remain ordinary transaction outcomes reproduced by all nodes.
...
- every successful mutation emits one independently replayable ADR-006 event while final overlay/index state is compacted per touched key;
...
- in ADR-007 schema v1 the final EVM state retains only direct commitment mappings and the storage schema marker at `0xEE0D`; ADR-008 replaces them with one persistent root slot;
...
- Exact production capacity remains subject to the later benchmark/limits stage; ADR-007 pins deterministic charging but makes no production throughput claim.
...
- ADR-008 can replace the direct-map backend inside the same seam without changing callers or same-block semantics.
...
Rejected because owner/day/global list queries would remain stale and would require the temporary same-block fence ADR-007 is intended to avoid.
...
Rejected because it undoes the ADR-005 body cutover and creates two persistent body authorities.
...
Crash/restart and full replay must see only finalized persistent commitments, never a committed block overlay. ExEx projection replay remains governed by ADR-004/006 and converges from canonical events.
...
ADR-007 is implemented on the same branch as ADR-005/006 and continues directly into ADR-008–010 without testnet activation. Schema v1 direct maps are a focused implementation/reference stage only; no genesis or network is started with them.

The single first CES1 deployment occurs after ADR-010 and combined verification. Its final genesis allocates marker-preserved `0xEE0D` using the then-current root/overlay schema, not this temporary direct-map layout. The address is added to the protocol registry, README, EIP-161 allowlist, and relevant architecture/debt records in that combined implementation change.
...
ADR-008 replaces the three direct maps with an authenticated unsharded CKB reference stage, then ADR-009/010 complete sharding and Root Catalog before the first testnet activation, while preserving the trusted domain interface, canonical events, same-block body/index overlay, and rollback semantics defined here.
