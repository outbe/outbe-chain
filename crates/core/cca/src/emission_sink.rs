...
pub fn distribute_daily(
    ...
    ctx.storage.with_checkpoint(|| {
        ...
        let records = contract
            ...
            .read_all()?
            .into_iter()
            .map(|cca| contract.load(cca))
            .collect::<Result<Vec<_>>>()?;
        let total = records.iter().try_fold(U256::ZERO, |total, record| {
            total
                .checked_add(record.reward_weight)
        ...
        for mut record in records {
            ...
            let wide_share =
                U512::from(amount) * U512::from(record.reward_weight) / U512::from(total);
            ...
            record.claimable_rewards = record
                .claimable_rewards
                .checked_add(native)
                .ok_or(CcaError::Arithmetic)?;
