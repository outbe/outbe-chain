...
///   1:  config_owner            — Address
///   2:  config_max_validators   — u32
///   3:  config_epoch_length_blocks — u32
///   4:  config_min_stake        — U256  [deprecated; Staking is the source of truth]
///   5:  config_is_initialized   — bool
///   6:  val_consensus_pubkey_lo — mapping(address => bytes32)  [BLS MinPk pubkey bytes 0..32]
///   7:  val_consensus_pubkey_hi — mapping(address => bytes32)  [BLS MinPk pubkey bytes 32..48, right-padded]
///   8:  val_stake               — mapping(address => uint256)
///   9:  val_status              — mapping(address => uint8)
///  10:  val_slash_count         — mapping(address => uint64)
///  11:  val_missed_blocks       — mapping(address => uint64)
///  12:  val_missed_votes        — mapping(address => uint64)
///  13:  val_blocks_proposed     — mapping(address => uint64)
///  14:  val_joined_at_height    — mapping(address => uint64)
///  15:  val_deactivated_at_height — mapping(address => uint64)
///  16:  val_unbonding_end       — mapping(address => uint64)
///  17:  address_to_index        — mapping(address => uint64)  [1-indexed]
///  18:  index_to_address        — mapping(uint64 => address)
///  19:  consensus_pubkey_hash_to_address — mapping(bytes32 => address)  [keccak256(48-byte pubkey)]
///  20:  _reserved_slot_19       — mapping(bytes32 => address)  [unused, was bls_pubkey_to_address]
///  21:  validator_count         — u32
///  22:  epoch_number            — U256
///  23:  epoch_start_timestamp   — u64
///  24:  epoch_start_block       — u64
///  25:  val_has_bls_share       — mapping(address => bool)
///  26:  pending_set_change      — bool
///  27:  active_consensus_set_hash — B256
///  28:  config_reregistration_cooldown — u32
///  29:  val_p2p_address_version — mapping(address => uint8)
///  30:  val_p2p_address_payload — mapping(address => bytes)
///  31:  finalized_participation_recorded — mapping(B256 => bool),
...
///  32:  committee_snapshot_exists                  — mapping(B256 => bool)
///       gates every snapshot read on a fully-written record. Keyed by the
///       canonical `snapshot_key`.
///  33:  committee_snapshot_len                     — mapping(B256 => u64)
///       length of the ordered committee at `snapshot_key`.
///  34:  committee_snapshot_address_at              — mapping(B256 => mapping(u64 => Address))
///       i-th validator address in canonical Commonware public-key order.
///  35:  committee_snapshot_pubkey_lo_at            — mapping(B256 => mapping(u64 => B256))
///       i-th BLS MinPk consensus pubkey bytes 0..32.
///  36:  committee_snapshot_pubkey_hi_at            — mapping(B256 => mapping(u64 => B256))
///       i-th BLS MinPk consensus pubkey bytes 32..48 right-padded with zeros.
///  37:  committee_snapshot_vrf_material_version    — mapping(B256 => u64)
///       VRF material version active when this snapshot was committed.
///  38:  committee_snapshot_vrf_group_public_key_hash — mapping(B256 => B256)
...
///  43:  config_unjail_cooldown_blocks — u64 (default 0)
///       blocks a JAILED validator must wait before `unjailValidator()`.
///  44:  val_jailed_at_height     — mapping(address => uint64)
///       block height the validator was JAILED; drives the unjail cooldown.
///  45:  committee_snapshot_key_ring — mapping(u64 => bytes32)
///       prune ring bounding the committee-snapshot store (slots 32..41).
///  46:  finalized_participation_ring — mapping(u64 => bytes32)
///       prune ring bounding the participation guard (slot 31).
///  47:  finalized_participation_ring_seq — u64
///       monotonic index counter for slot 46.
///  48:  committee_snapshot_vrf_public_polynomial_hash — mapping(B256 => B256)
...
pub struct ValidatorSet {
    ...
    // Per-validator fields keyed by Address (slots 6-16)
    ...
    // Versioned Commonware P2P address registry (slots 29-30)
    ...
    // (slot 31). Keyed by `metadata.finalized_block_hash`; set on first
    ...
    // V2 `CommitteeSnapshotStore` (slots 32..41).
    ...
    /// Slot 32 — existence flag, gates every read path on a fully-written record.
    pub committee_snapshot_exists: Mapping<B256, bool>,
    /// Slot 33 — length of the ordered committee.
    pub committee_snapshot_len: Mapping<B256, u64>,
    /// Slot 34 — `i`-th validator address in Commonware public-key order.
    pub committee_snapshot_address_at: Mapping<B256, Mapping<u64, Address>>,
    /// Slot 35 — BLS MinPk pubkey bytes 0..32 for entry `i`.
    pub committee_snapshot_pubkey_lo_at: Mapping<B256, Mapping<u64, B256>>,
    /// Slot 36 — BLS MinPk pubkey bytes 32..48, right-padded with zeros.
    pub committee_snapshot_pubkey_hi_at: Mapping<B256, Mapping<u64, B256>>,
    /// Slot 37 — `vrf_material_version` active when the snapshot was committed.
    pub committee_snapshot_vrf_material_version: Mapping<B256, u64>,
    /// Slot 38 — `keccak256(commonware_codec::Encode(polynomial.public()))`.
    pub committee_snapshot_vrf_group_public_key_hash: Mapping<B256, B256>,
    /// Slot 39 — length in bytes of the encoded VRF group public key.
    pub committee_snapshot_vrf_group_public_key_len: Mapping<B256, u64>,
    /// Slot 40 — 32-byte chunks of `commonware_codec::Encode(polynomial.public())`.
    /// Last chunk is right-padded with zeros; readers trim using slot-39 length.
    pub committee_snapshot_vrf_group_public_key_chunk_at: Mapping<B256, Mapping<u64, B256>>,
    /// Slot 41 — reserved for future snapshot pruning metadata. Genesis V2
    /// requires this to remain zero.
    pub _reserved_committee_snapshot_slot_40: Slot<B256>,

    /// Slot 42 — stale-join activation guard. A PENDING joiner is included in
    ...
    /// Slot 43 — unjail cooldown in blocks. A JAILED validator may call
    ...
    /// Slot 44 — block height at which a validator was JAILED (set by
    ...
    /// Slot 45 — committee-snapshot prune ring (`epoch % COMMITTEE_SNAPSHOT_RETAIN_EPOCHS
    ...
    /// (slots 32..41) to the last `COMMITTEE_SNAPSHOT_RETAIN_EPOCHS` epochs instead
    ...
    /// Slot 46 — finalized-participation guard prune ring (`seq %
    /// FINALIZED_PARTICIPATION_RETAIN → fb_hash`). Bounds slot 31
    ...
    /// Slot 47 — monotonic counter for the slot-46 ring index (incremented once
    /// per distinct finalized block recorded).
    pub finalized_participation_ring_seq: Slot<u64>,

    /// Slot 48 — `keccak256(Encode(full public polynomial))` per committee
    /// snapshot, keyed by `snapshot_key` (same key as slots 32..41). Written by
