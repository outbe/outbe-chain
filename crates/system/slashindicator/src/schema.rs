...
///   1: config_proposer_misdemeanor_threshold — u64 (default 50)
///   2: config_proposer_felony_threshold      — u64 (default 150)
///   3: config_voter_misdemeanor_threshold    — u64 (default 500)
///   4: config_slash_amount_percent           — u64 (default 5)
///   5: config_evidence_reward_percent        — u64 (default 10)
///   6: proposer_miss_count                   — mapping(address => u64), per-epoch, resets
///   7: voter_miss_count                      — mapping(address => u64), per-epoch, resets
///   8: felony_count                          — mapping(address => u64), cumulative
///   9: evidence_processed                    — mapping(B256 => bool), dedup
///  10: voter_window_slashed                  — mapping(B256 => bool), per-finalized-block voter slash-window guard
///  11: proposer_window_slashed               — mapping(B256 => bool), per-finalized-block missed-proposer slash-window guard
///  12: invalid_vrf_evidence_processed        — mapping(B256 => bool) dedup keyed by `invalid_vrf_evidence_hash_v2(child_hash, phase1_tx_hash)`
///  13: config_voter_felony_threshold         — u64 (default 150); appended at the end to preserve the slot 1-12 layout
///  14: seed_partial_equivocation_processed   — mapping(B256 => bool) dedup keyed by `SeedPartialEquivocationEvidence::dedup_hash`
///  15: invalid_seed_partial_processed        — mapping(B256 => bool) dedup keyed by `InvalidSeedPartialEvidence::dedup_hash`
///  16: slash_guard_ring                      — mapping(uint64 => B256), prune ring of finalized fb_hashes
///  17: slash_guard_ring_seq                  — uint64, ring write cursor
...
pub struct SlashIndicator {
    ...
    // Per-validator miss counters (slots 6-7), reset each epoch
    pub proposer_miss_count: Mapping<Address, u64>,
    pub voter_miss_count: Mapping<Address, u64>,

    // Cumulative felony count (slot 8), never reset
    pub felony_count: Mapping<Address, u64>,

    // Evidence dedup — tracks processed evidence hashes (slot 9)
    ...
    // per-voter nested mapping. Pruned by the `slash_guard_ring` (slots 16/17).
    ...
    // conflicting-vote evidence (slot 9).
    pub invalid_vrf_evidence_processed: Mapping<B256, bool>,

    // Config (late addition, slot 13): voter felony threshold. Appended at the
    // end so existing slots 1-12 keep their layout. `slash_voter` force-exits
    ...
    // Dedup guard for `submitSeedPartialEquivocationEvidence` (slot 14). Key is
    ...
    // Dedup guard for `submitInvalidSeedPartialEvidence` (slot 15). Key is
    ...
    // prune ring (slots 16/17) bounding `voter_window_slashed` and
