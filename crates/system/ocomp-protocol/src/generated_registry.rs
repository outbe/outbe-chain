...
pub const FIDELITY_OPENING_CODEC_DESCRIPTOR: &str = "role=FIDELITY_OPENING;version=1;record=AuthenticatedOpeningV1(source_kind=1,subject,value,codec_id,opening);subject=u32_be(owner_count)||address20*;owner_count=1..256;owners=strict-ascending-unique;value=u32_be(slot_count)||(slot:B256||value:U256_be)*;contract=METADOSIS_ADDRESS;slots=metadosis_league_snapshot_slots_v1(base=29,key=keccak256(0x4c||u32_be(wwd)||address20),mapping=keccak256(key||slot32),one-slot-per-owner-in-subject-order);league=nonzero-u16-frozen-at-ready-preparation;opening=address20||state_root:B256||u32_be(slot_count)||(slot:B256||value:U256_be)*||u32_be(account_proof_len)||account_proof||u32_be(storage_proof_len)||storage_proof;proof=RETH_ETHEREUM_ACCOUNT_STORAGE_MPT_V1;rules=state_root_equals_checkpoint,exact-contract,exact-slot-plan,strict-decode-reencode";
pub const FIDELITY_OPENING_CODEC_ID: alloy_primitives::B256 = alloy_primitives::B256::new([124, 70, 12, 35, 45, 51, 25, 106, 58, 156, 81, 40, 132, 114, 123, 241, 21, 81, 209, 215, 254, 88, 201, 180, 35, 124, 182, 187, 38, 214, 33, 38]);

pub const ORACLE_OPENING_CODEC_DESCRIPTOR: &str = "role=ORACLE_OPENING;version=2;record=AuthenticatedOpeningV1(source_kind=2,subject,value,codec_id,opening);subject=u32_be(wwd)||u16_be(iso_count)||u16_be(iso)*;isos=strict-ascending-unique-nonzero-and-contains-840;value=u32_be(slot_count)||(slot:B256||value:U256_be)*;contract=NOD_ADDRESS;slots=nod_entry_price_slots_v1(frozen=30,price=33,mapping=keccak256(key||slot32),price_key=(wwd,iso),isos<=256);price=previous-utc-day-vwap-frozen-at-ready-preparation;missing-or-zero-input=unpriced;opening=address20||state_root:B256||u32_be(slot_count)||(slot:B256||value:U256_be)*||u32_be(account_proof_len)||account_proof||u32_be(storage_proof_len)||storage_proof;proof=RETH_ETHEREUM_ACCOUNT_STORAGE_MPT_V1;rules=state_root_equals_checkpoint,exact-contract,exact-slot-plan,frozen-flag-is-one,strict-decode-reencode";
pub const ORACLE_OPENING_CODEC_ID: alloy_primitives::B256 = alloy_primitives::B256::new([88, 1, 48, 2, 233, 179, 254, 98, 175, 94, 74, 135, 58, 103, 95, 24, 38, 89, 67, 234, 102, 55, 148, 126, 53, 33, 198, 171, 63, 94, 187, 124]);

pub const OPENING_CODEC_REGISTRY_HASH: alloy_primitives::B256 = alloy_primitives::B256::new([95, 86, 19, 182, 199, 85, 66, 201, 193, 187, 178, 243, 244, 110, 132, 7, 157, 141, 89, 76, 189, 166, 39, 157, 209, 226, 138, 101, 106, 171, 19, 26]);
