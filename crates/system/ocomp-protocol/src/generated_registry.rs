...
pub const ORACLE_OPENING_CODEC_DESCRIPTOR: &str = "role=ORACLE_OPENING;version=1;record=AuthenticatedOpeningV1(source_kind=2,subject,value,codec_id,opening);subject=u32_be(wwd)||u16_be(iso_count)||u16_be(iso)*;isos=strict-ascending-unique-and-contains-840-and-subset-of-reference-currencies;value=u32_be(slot_count)||(slot:B256||value:U256_be)*;slots=oracle_slot_plan_v1(pair_ordinal=10,scurve_count=34,scurve_pair_base=35,scurve_pair_quote=67,scurve_peak_day=36,scurve_peak_price=37,scurve_oldest=38,wwd_vwap_exists=47,wwd_vwap_pair_count=50,wwd_vwap_pair_base=51,wwd_vwap_pair_quote=68,wwd_vwap_value=52,reference_currencies=55,mapping=keccak256(key||slot32),vec=keccak256(slot32)+index,pair_key=sorted_concat(address20(coen=0x00*20),address20(iso=0x000cc||bcd3(iso))),wwd_pairs<=256,active_scurve<=256,reference_currencies<=256,isos<=256);opening=address20||state_root:B256||u32_be(slot_count)||(slot:B256||value:U256_be)*||u32_be(account_proof_len)||account_proof||u32_be(storage_proof_len)||storage_proof;proof=RETH_ETHEREUM_ACCOUNT_STORAGE_MPT_V1;rules=state_root_equals_checkpoint,exact-contract,exact-slot-plan,strict-decode-reencode";
pub const ORACLE_OPENING_CODEC_ID: alloy_primitives::B256 = alloy_primitives::B256::new([5, 4, 1, 47, 5, 92, 240, 238, 184, 164, 157, 100, 155, 110, 21, 31, 237, 225, 95, 255, 24, 203, 127, 40, 81, 63, 61, 49, 17, 16, 148, 75]);

pub const OPENING_CODEC_REGISTRY_HASH: alloy_primitives::B256 = alloy_primitives::B256::new([61, 20, 8, 44, 64, 163, 139, 39, 168, 229, 71, 105, 2, 200, 205, 69, 252, 32, 195, 16, 204, 82, 150, 11, 4, 150, 145, 182, 213, 137, 122, 207]);
