...
contract OFTDeploy is Script {
    ...
    {
        return abi.encodePacked(type(USDT0OFT).creationCode, abi.encode(name_, symbol_, decimals_, vm.envAddress("DEPLOYER_ADDRESS")));
    }
    ...
    function _setTokenBridge(uint256 pk, address signer, address token, address tokenBridge) internal {
        ...
        USDT0OFT bridgeableToken = USDT0OFT(token);
