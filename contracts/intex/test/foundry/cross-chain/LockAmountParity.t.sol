...
///         `qty * basis * rate / RATE_SCALE` in 256-bit space with `basis = promis_load`. A
...
contract LockAmountParityTest is Test {
    ...
    function bnbLockAmount(uint16 quantity, uint128 basis, uint32 rate) external pure returns (uint128) {
        uint256 wide = uint256(quantity) * basis * rate / RATE_SCALE;
    ...
    function test_Parity_AtPromisLoadBasis() public view {
        // qty = uint16 max, rate = RATE_SCALE (100% of basis) → the largest live lock at this basis.
    ...
    function test_Parity_AtUint128Boundary() public view {
        // basis = uint128 max, qty = 1, rate = RATE_SCALE → product = basis, the boundary value.
        assertEq(
