...
description: >
  Events/errors sufficiency audit for the intex contracts —
  judgment-based, interface-surface only. Ensures every privileged
  state rotation emits an event, event payloads are
  log-reconstructible (acted-on key, amounts, old+new value), errors
  follow one-error-one-failure (overloaded errors split, dead errors
  removed), and event/error naming is consistent. Changes are confined
  to event/error declarations and the error type at each existing
  revert site: it never changes WHEN a revert fires, what state
  changes, storage layout, role gating, function selectors, or
  cross-chain message format. EIP-170 size-gated, and every rename is
  propagated to TS (tasks/scripts) and tests in lockstep. Use whenever
  Solidity event/error observability or naming needs review, on one
  contract or the whole set. Companion to solidity-cosmetics-natspec,
...
everything from the current code (no point-in-time finding list). Authoritative
convention: **`docs/solidity-conventions.md`**; this skill is its executable
encoding for events/errors and stays self-sufficient if absent.

Work from `contracts/intex/` (its own `foundry.toml`, `package.json`,
`.solhint.json`). Uses **yarn**; relevant scripts: `compile`,
`test:forge`/`test:hardhat`/`test`, `slither`, `aderyn`, `analyze`,
`cd:extract-abi`, `format:fix`, `lint`.

## Target selection
Discover, never hardcode (identical to the companion skill):
```
# one contract:
...
```
`vendor/`, `interfaces/`, `Mock*`, and test scaffolding are out of scope. Events
and errors live on the impl and must also be declared on its interface; edit both.
...
- cross-chain message encoding/decoding (the `*MsgCodec` libraries — their
  event/error surface falls here, but the wire format must not move),
...
```
cd contracts/intex
forge build && yarn test         # green before you start
...
```
Cross-check declarations vs usage on the impl:
```
rg -n 'emit |revert |error |event ' contracts/<dir>/<Contract>.sol
```
...
- Remove error declarations with zero revert sites (`rg -nw '<Error>' contracts`).
- Keep error encoding economical — this repo budgets custom-error size against
  EIP-170 (see `docs/adr/0006-custom-error-encoding-eip170-budget.md`); prefer
  few parameters over verbose ones on size-critical contracts.
...
```
forge build --sizes
forge test --match-path 'test/foundry/*sizes*'   # if size tests exist (messengers)
```
Adding events/errors grows bytecode. No contract may cross the 24,576-byte limit
(messengers run closest). If a change would breach it, economize the encoding or
defer the addition and record why.

### 6. Propagate every rename/added-field in lockstep
A changed event/error selector or shape ripples off-chain and into tests:
```
rg -nw '<OldEventOrErrorName>' tasks scripts test config
yarn compile          # Hardhat + TypeChain regen (must succeed)
yarn cd:extract-abi   # refresh exported ABI artifacts
```
Update TS consumers, `expectRevert`/`expectEmit` (Foundry) and Hardhat assertions
to the new names/shapes. A stale reference is a failed run.
...
forge build && yarn compile                        # Foundry + Hardhat/TypeChain
forge build --sizes                                # no contract over EIP-170
yarn test                                          # forge + hardhat green with updated expectations
yarn analyze                                       # slither + aderyn: no NEW high/critical vs baseline
rg -nw '<AnyOldName>' tasks scripts test config    # zero stale references
```
