...
/// @title USDTDeploy
/// @notice ERC-7786 / ERC-7802 deployment and configuration script for USDT: canonical on the external chain,
///         ERC-7802 synthetic on Outbe.
/// @dev The external chain is whichever chain `EXTERNAL_CHAIN_ID` / `EXTERNAL_USDT_*` point at — BNB testnet,
///      Sepolia, anvil. No chain id and no network name is hardcoded: adding a network is an env change.
contract USDTDeploy is Script {
    ...
    ///      deployed on the chain the operator declared as the external chain. Any other chain (a wrong `--rpc-url`, a
    ...
    // The salt strings still read "USDT0" — the deployed token's symbol. They are inputs to the CREATE2 address, so
    // renaming them would move every already-deployed token and bridge to a different address.
    ...
    function deploySource() external returns (address sourceToken, address tokenBridge) {
        ...
        address configuredToken = vm.envOr("EXTERNAL_USDT_TOKEN", address(0));
        address configuredBridge = vm.envOr("EXTERNAL_USDT_BRIDGE", address(0));
        ...
        console2.log("EXTERNAL_USDT_TOKEN=", sourceToken);
        console2.log("EXTERNAL_USDT_BRIDGE=", tokenBridge);
    }
    ...
    function setTargetTokenBridge() external {
        uint256 pk = _getPrivateKey();
        address signer = vm.addr(pk);
        _setTokenBridge(pk, signer, vm.envAddress("OUTBE_USDT_TOKEN"), vm.envAddress("OUTBE_USDT_BRIDGE"));
    }

    function configureSourceRemote() external {
        _configureRemote(
            vm.envAddress("EXTERNAL_USDT_BRIDGE"), vm.envUint("OUTBE_CHAIN_ID"), vm.envAddress("OUTBE_USDT_BRIDGE")
        );
    }

    function configureTargetRemote() external {
        _configureRemote(
            vm.envAddress("OUTBE_USDT_BRIDGE"), vm.envUint("EXTERNAL_CHAIN_ID"), vm.envAddress("EXTERNAL_USDT_BRIDGE")
        );
    }
    ...
    function _setTokenBridge(uint256 pk, address signer, address token, address tokenBridge) internal {
        ...
        if (!_shouldBroadcastOwnerCall(signer, owner, token, safeTxData, "Set USDT token bridge")) return;

        vm.startBroadcast(pk);
        bridgeableToken.setTokenBridge(tokenBridge);
        vm.stopBroadcast();

        console2.log("USDT token bridge set:", tokenBridge);
    }
    ...
    function _configureRemote(address localTokenBridge, uint256 remoteChainId, address remoteTokenBridge) internal {
        ...
        if (!_shouldBroadcastOwnerCall(signer, owner, localTokenBridge, safeTxData, "Configure USDT remote bridge")) {
            return;
        }
    ...
    function _logTarget(TargetDeployment memory target) internal pure {
        console2.log("OUTBE_USDT_TOKEN=", target.token);
        console2.log("OUTBE_USDT_BRIDGE=", target.tokenBridge);
