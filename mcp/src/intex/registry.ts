...
export const NFT_ABI: Abi = parseAbi([
  ...
  "function tokenIds(bytes14 seriesId) view returns (uint256 issued, uint256 settled)",
  "function readData(bytes14 seriesId) view returns ((uint16 issuanceCurrency, uint16 referenceCurrency, uint32 issuedIntexCount, uint128 promisLoadMinor, uint64 entryPriceMinor, uint64 floorPriceMinor, uint64 callPriceMinor, (uint32 callWindow, uint32 callThreshold, uint32 callNoticePeriod) callTrigger, uint32 issuedAt, uint32 calledAt, uint32 totalSupply, uint8 status, uint8 state, uint32 worldwideDay) data)",
...
export const INTEX_ABI: Abi = parseAbi([
  ...
  "function seriesAt(uint64 index) view returns (bytes14)",
]);
...
export const FACTORY_ABI: Abi = parseAbi([
  ...
  "function quoteCostAmount(bytes14 seriesId, address paymentToken) view returns (uint256 costAmountMinor)",
  "function minePromis(bytes14 seriesId, uint256 amount, uint256 nonce, bytes32 mac, uint64 opNonce) returns (uint256 promisAmount)",
  "function setAuthorizedSettler(bytes14 seriesId, address settler)",
