...
| `testing/e2e-harness/src/features/ocomp/setup.rs` | modify | Select the existing scenario-local felony-threshold override for the intentional donor outage; keep other scenarios unchanged. |
| `testing/e2e-harness/src/features/price_oracle.rs` | test only | Verify that rebuilding the existing Oracle cohort binds a restarted donor PID and requires a publication newer than its finalized checkpoint. |
| `bin/outbe-chain/tests/common/snapshot.rs`, `bin/outbe-chain/tests/snapshot_validate.rs` | test-only correction | Share the existing file-backed 600-second CLI watchdog for native fixtures containing a 4 GiB CE file. |
...
4. Observe actual H/hash and native chain/OCOMP progress, continuing finality, saved Tribute/NOD and completed result reuse, plus pending public materialization/payout. Read each complete public NOD owner tuple against a stable head. Within one bounded scan deadline, discard successful mixed-head tuples and retry only the observed adjacent, unchanged-root CE transition with matching parent identity and independently observed forward head movement. Keep shared RPC error propagation and all missing/uniqueness/body assertions unchanged; log retries and fail on deadline exhaustion. Task08 fixtures cover native cursor lag independently of quiet-cut timing. No claim that a new node must resume the donor unfinished Lysis attempt under another identity.
5. Perform a new genuine job/block progression after file placement, reach K>H, stop normally, and restart with ordinary node command. The harness advances its existing controlled clock to the native next-day forming boundary if that day has not been created, waits for ProtocolCycle to create it, and reads its actual genesis-derived phase schedule before submitting work. Consume the existing post-clock Oracle publication requirement together with each lifecycle observation: actual day creation, current offering before submitting Tribute, and the retained finalized new JobIntent. Compare result/NOD/public effects and prove current-K recovery without original artifact. Original-cut file hashes are not current-K invariants; a later normal restart never reads the old manifest/report.
...
7. The offline-snapshot localnet uses the existing genesis felony threshold of epoch length minus one (299 of 300 blocks), allowing the intentional donor outage across two file-placement variants. Both donor outages use the existing Oracle feeder stop/resume helpers: retain the old finalized publication barrier before stopping the donor, require fresh common finality after it resumes, rebuild the cohort with current process identities, and observe its newer publication. Recipient-only restarts do not change Oracle membership. Other OCOMP scenarios retain their usual threshold. Ordinary jail/TEE rules and production configuration are unchanged. Verify the existing bootstrap unit tests and the effective generated genesis.
8. Finish harness preflight before expensive release build; rebuild only changed runtime inputs. Use current release binaries, sudo, real gramine-sgx, --tee sgx-no-attest, chain_id54322345, no DCAP/QVL. No invented service UID model.
9. Run fmt, workspace unit/doctests, applicable feature lanes and zero-warning Clippy. Independently review diff against main for every retained caller, native data coverage and no live-capture/bootstrap/transport dependencies. Record exact commands/counts/revisions and known baseline failures honestly.
