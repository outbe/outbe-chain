...
| `crates/blockchain/snapshot/src/manifest.rs` | new | SnapshotManifestV1, NativeProgress, DomainInventory, FileEntry and structural validation; finite logical domains, exact raw manifest digest and sorted member order. |
...
8. Freeze the format before downstream work: version, chain/genesis, finalized H/hash, actual progress observations, producer/time/optional source attestation, explicit domain inventory, relative paths/sizes/SHA256, required empty-domain records. No host absolute path grants extraction authority.
...
2. Unit first: manifest rejects duplicate/absolute/parent paths, unknown domain/version, duplicate fields, inconsistent counts and invalid hashes; differing H/C/projection fields round-trip without normalization.
