...
2. Enumerate/hash regular data files and preserve the recorded inventory order; readers do not require lexical sorting. Reject symlink/hardlink/special entries and source/output aliases; open relative to anchored directory handles. Do not recursively copy configuration or all of consensus/OCOMP root.
3. Manifest bytes are serialized struct/lists in the recorded order; verification uses those exact bytes without sorting or reserialization. Bundle ID = SHA256(raw manifest bytes). Signing digest = SHA256(b"outbe/snapshot/manifest/v1\0" || bundle_id). Envelope contains version, scheme, bundle_id, compressed public key and 65-byte recoverable low-S secp256k1 signature. Use existing OutbeEvmSigner and recovery primitives; no new key custody.
