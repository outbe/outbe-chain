...
| Process lifecycle testing | E2E `ChildGuard`, validator/enclave start-stop-restart controls and retained logs | Extend the existing harness to own external OCOMP processes and observe the embedded Supervisor | Add Exporter/Worker/CAS handles, typed external-process faults and node-owned Supervisor HTTP status probes |
