...
use crate::internal::{
    ...
    config::Config,
    eth::{
        self, IGovernance, IL2Registry, INod, ISlashIndicator, IStaking, ITeeRegistry, ITribute,
        IUpdate, IValidatorSet, IVote, IWorldwideDay, IZeroFee,
    },
...
impl Rpc {
    ...
    pub fn validator_record(&self, port: u16, addr: &str) -> Option<ValidatorRecord> {
        ...
        Some(ValidatorRecord {
            address: record.addr,
            consensus_pubkey: record.pubkey,
            stake: record.stake,
            ...
            joined_at_height: record.joined,
            deactivated_at_height: record.deactivated,
            unbonding_end: record.unbondEnd,
            has_bls_share: record.hasShare,
        })
    }
    ...
    pub fn is_validator(&self, port: u16, addr: &str) -> Option<bool> {
        ...
        eth::read_call(
            &self.url(port),
            addresses::VS_ADDR,
            &IValidatorSet::isValidatorCall { v },
        )
    }
    ...
    pub fn validator_p2p_address(&self, port: u16, addr: &str) -> Option<ValidatorP2pAddress> {
        ...
        let value = eth::read_call(
            &self.url(port),
            addresses::VS_ADDR,
            &IValidatorSet::getP2pAddressCall { v },
        )?;
    ...
    /// SlashIndicator's cumulative voter-miss counter.
    pub fn voter_miss_count(&self, port: u16, addr: &str) -> Option<u64> {
        let validator = addr.parse().ok()?;
        eth::read_call(
            &self.url(port),
            addresses::SLASH_ADDR,
            &ISlashIndicator::getVoterMissCountCall { validator },
        )
    }
    ...
    pub fn register_validator(
        ...
        eth::send_call_outcome(
            ...
            &IValidatorSet::registerValidatorCall {
                v: validator,
                pubkey: Bytes::copy_from_slice(consensus_pubkey),
                sig: Bytes::copy_from_slice(bls_signature),
            },
            None,
        )
        .map(Into::into)
    ...
    pub fn set_validator_p2p_address(
        ...
        eth::send_call_outcome(
            ...
            &IValidatorSet::setP2pAddressCall {
                v: validator,
                kind: version,
                addr: Bytes::copy_from_slice(encoded),
            },
            None,
        )
        .map(Into::into)
    ...
    pub fn activate_reshared_set(
        ...
        eth::send_call_outcome(
            ...
            addresses::VS_ADDR,
            caller_key,
            &IValidatorSet::activateResharedSetCall {
