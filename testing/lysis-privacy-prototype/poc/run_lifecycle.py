...
LIMITER = HERE.parent / "measurements/run_with_ram_limit.py"
...
class Run:
    def __init__(self, out, count, su):
        ...
        self.source_paths=sorted(list(HERE.glob("*.py"))+list((HERE/"src").rglob("*.rs"))+list((HERE/"source-helper/src").rglob("*.rs"))+list((HERE.parent/"measurements/p-link/src").rglob("*.rs"))+[HERE/"Cargo.lock",HERE/"Cargo.toml",HERE/"source-helper/Cargo.lock",HERE/"source-helper/Cargo.toml",REPO/"crates/core/lysis/src/algorithm.rs",REPO/"crates/core/fidelity-math/src/lib.rs"])
    ...
    def job(self, binary, name, body, **kw):
        ...
        return self.command(name, [BIN/binary, filename], **kw)
    ...
    def mpc(self, tag, plan):
        ...
        for i in range(3):
            ...
            logs.append(log)
            processes.append(subprocess.Popen([str(HERE/".venv-mpc/bin/python"), str(HERE/"mpc_worker.py"), plan_path, "-M3", "-T1", "-I", str(i), "--base-port", "17430"], cwd=REPO, env=self.env, stdout=log, stderr=subprocess.STDOUT))
        try:
    ...
    def transition(self, tag, kind, prepare, context):
        ...
        elif "owner" not in context:raise ValueError("wallet operation needs its registered owner")
        if coupled:self.cohort.bind_context(context)
        private = self.private / f"wallet-operation-{tag}"
        public = self.public / f"operation-{tag}"
        self.job("poc-state", "wallet-prepare", {"op":"prepare","kind":kind,"out":str(private),"context":context,**prepare})
        parameters = HERE / f"parameters/{kind}-v2"
        if not (parameters/"pk.bin").exists():
            self.job("poc-state", f"setup-{kind}", {"op":"setup","kind":kind,"dir":str(parameters)}, limit=2_000_000_000)
        self.job("poc-state", f"cold-{kind}-proof", {"op":"prove","parameters":str(parameters),"witness":str(private/"transition.private.json"),"out":str(public)}, limit=WALLET_LIMIT)
        self.job("poc-state", f"verify-{kind}", {"op":"verify","parameters":str(parameters),"statement":str(public/"statement.public.json"),"proof":str(public/"proof.bin"),"out":str(public/"verify.json"),"mutate":True})
        statement=read(public/"statement.public.json")
        wallet=self.wallets_by_owner[context["owner"]]
        if read(wallet/"identity.json")!=read(self.public/"source-registry.json")["wallet_owners"][context["owner"]]:raise ValueError("registered wallet key changed")
        ...
        self.verified_statements[sha(statement)]=context["owner"]
    ...
    def commit_transition(self, tag, kind, statement, context, input_ids, output_ids, owner, asset="gratis", nod=None):
        ...
        # to the proof. Groth16 verification has already succeeded in a node-only
    ...
    def run(self):
        ...
        if not (parameters/"pk.bin").exists():
            self.command("link-setup",[BIN/"outbe-private-lifecycle-poc","setup",parameters,self.su],limit=2_000_000_000)
        wallets=[]; l2_rows=[]; link_rows=[]
        for i in range(self.count):
            wallet=self.private/f"wallet-{i}"; self.command("wallet",[BIN/"outbe-private-lifecycle-poc","wallet",wallet,self.su,i+1000])
        ...
        print("real P_L2 ready; proving distinct P_link batch",flush=True)
        self.command("warm-distinct-link",[BIN/"outbe-private-lifecycle-poc","batch-prove",parameters,link_manifest],limit=WALLET_LIMIT)
        self.command("node-l2-verify",[L2,"verify-batch",l2_manifest],limit=2_000_000_000)
        self.command("node-link-verify",[BIN/"outbe-private-lifecycle-poc","batch-verify",parameters,link_manifest],limit=2_000_000_000)
        # Also measure a fresh wallet with the full source+P_L2-bound public data.
        self.command(f"cold-link-{self.su}",[BIN/"outbe-private-lifecycle-poc","prove",parameters,wallets[0]/"wallet.private.json",self.public/"cold-link"],limit=WALLET_LIMIT)
        ...
        for i,o in enumerate(offers):
            raw=(Path(l2_rows[i]["out"])/"p_l2.bin").read_bytes()
