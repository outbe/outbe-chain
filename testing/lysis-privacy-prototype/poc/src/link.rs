...
use crate::crypto::{commit, generators, scalar_integer};
use ark_bn254::Fr;
use ark_ed_on_bn254::{constraints::EdwardsVar, EdwardsAffine, Fr as Scalar};
...
impl LinkPublic {
    ...
    pub fn inputs(&self) -> Result<Vec<Fr>, String> {
        ...
        v.push(self.commitment.x);
        v.push(self.commitment.y);
...
pub fn fixture(capacity: usize, index: u64) -> LinkCircuit {
    ...
    let blinder = Scalar::rand(&mut OsRng);
...
impl ConstraintSynthesizer<Fr> for LinkCircuit {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fr>) -> Result<(), SynthesisError> {
        ...
        let cx = public_fp(cs.clone(), p.commitment.x, &mut all)?;
        let cy = public_fp(cs.clone(), p.commitment.y, &mut all)?;
        ...
        // Source bound a<2^104 is below the Baby scalar modulus. The commitment
        // consumes exactly the quotient bits; no independent reduced witness.
        let (g, h) = generators();
        let gv = EdwardsVar::new_constant(cs.clone(), g)?;
        let hv = EdwardsVar::new_constant(cs.clone(), h)?;
        let r =
            outbe_p_link_measurements::integer::bits(cs.clone(), &scalar_integer(w.blinder), 251)?;
        less_constant(&r, &crate::crypto::scalar_modulus())?.enforce_equal(&Boolean::TRUE)?;
        let actual = gv.scalar_mul_le(a.bits.iter())? + hv.scalar_mul_le(r.iter())?;
        actual.x.enforce_equal(&cx)?;
        actual.y.enforce_equal(&cy)?;
        Ok(())
    }
}
