...
class TwistedRun(Run):
    def __init__(self,*args):
        ...
        self.source_paths=sorted(set(self.source_paths))
    ...
    def twisted(self,name,body,**kw):
        ...
        return self.command(name,[V2/"target/release/ristretto-money",job],**kw)
    ...
    def finish(self,*args):
        ...
        report.update(backend="full-ristretto-twisted-bulletproofs-sigma",vss_group="Ristretto255",mpc_share_field="Ristretto scalar order",same_history_arithmetic=True,retained_economics_groth16=[],cross_curve_bridges=0,wallet_operations={p.parent.name:read(p) for p in self.public.glob("operation-*/twisted-resources.json")},cipher_registry_bytes=self.cipher_registry.stat().st_size,cipher_registry_entries=len(read(self.cipher_registry)),encryption_key_count=len(self.cipher_keys))
