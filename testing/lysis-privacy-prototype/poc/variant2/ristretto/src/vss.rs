...
use curve25519_dalek::{ristretto::RistrettoPoint as EdwardsAffine, scalar::Scalar as Fr};
...
pub fn point_hex(p: EdwardsAffine) -> Result<String> {
    Ok(hex::encode(p.compress().to_bytes()))
}
...
pub fn eval(p: &Polynomial, x: u32) -> Result<EdwardsAffine> {
    ...
    Ok(point(&p.points[0])? + point(&p.points[1])? * Fr::from(x))
}
...
pub fn lagrange(xs: &[u32], i: usize) -> Result<Fr> {
    ...
    if b == Fr::ZERO {
        return Err("duplicate share coordinate".into());
    }
    Ok(a * b.invert())
}
