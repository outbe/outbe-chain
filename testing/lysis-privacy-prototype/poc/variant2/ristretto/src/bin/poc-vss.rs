...
use curve25519_dalek::{ristretto::RistrettoPoint, scalar::Scalar as Fr};

use outbe_ristretto_lifecycle_poc::{crypto::*, vss::*};
...
fn run() -> Result<()> {
    ...
    match j["op"].as_str().ok_or("op missing")? {
        ...
        "interpolate-outputs" => {
            ...
            let ps = (0..4)
                .map(|i| {
                    ...
                    let c0 = a * Fr::from(2u32) - b;
                    let c1 = b - a;
                    if (c0 + c1 * Fr::from(3u32)) != point(&points[2][i])? {
        ...
        "recover-note" => {
            ...
            write_private_json(
                path(&j, "out")?,
                &outbe_ristretto_lifecycle_poc::state::Note {
        ...
        "wallet-secrets" => {
            let w: outbe_ristretto_lifecycle_poc::wire::Wallet = read(path(&j, "wallet")?)?;
            ...
            if let Some(notes) = j["notes"].as_array() {
                for n in notes {
                    let note: outbe_ristretto_lifecycle_poc::state::Note =
        ...
        "rotate-deal" => {
            ...
            for r in old.records.values() {
                ...
                if point(&p.points[0])? != (eval(&r.polynomial, x)? * l) {
            ...
            for (i, f) in files.iter().enumerate() {
                ...
                for (r, p) in rr.iter().zip(&prior) {
                    ...
                    if point(&r.polynomial.points[0])?
                        != (eval(p, selected[i])? * lagrange(&selected, i)?)
                    {
        ...
        "open" => {
            ...
            for (g, ids) in groups {
                let mut points = [RistrettoPoint::default(); 2];
                for id in ids {
