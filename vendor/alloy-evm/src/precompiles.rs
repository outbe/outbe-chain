...
pub struct PrecompilesMap {
    ...
    /// Outbe fork extension. Optional context-dispatch hook called BEFORE the
    /// standard precompile lookup / `Context` destructuring path. Stored as a
    /// pair of type-erased closures: a fast-path filter (`handles`) and a
    /// dispatcher that receives the raw `*mut Context<...>` along with the
    /// call inputs.
    ///
    /// Registered via [`PrecompilesMap::set_ctx_dispatch_hook`]; the typed
    /// `&mut CTX` is reconstructed inside the wrapper closure that the
    /// setter installs. See the setter's safety contract.
    #[allow(clippy::type_complexity)]
    ctx_dispatch: Option<(
        Arc<dyn Fn(&Address) -> bool + Send + Sync>,
        Arc<
            dyn Fn(
                    *mut core::ffi::c_void,
                    &CallInputs,
...
impl PrecompilesMap {
    ...
    /// Outbe fork extension. Install a context-dispatch hook.
    ///
    /// When set, `handles(addr)` is consulted on every
    /// [`PrecompileProvider::run`] before the standard precompile lookup. If
    /// it returns `true`, `dispatch(ctx_ptr, inputs)` is invoked with a raw
    /// pointer to the unbroken `&mut Context<...>` cast as
    /// `*mut core::ffi::c_void`. The caller MUST cast back to the same
    /// concrete `Context<...>` type that [`PrecompileProvider::run`] is
    /// specialised for on this `PrecompilesMap` instance. This enables
    /// stateful precompiles to drive child CALL frames via the full revm
    /// handler (e.g. `revm_handler::EthFrame::make_call_frame`) which
    /// requires `&mut Context<...>`.
    ///
    /// # Why raw pointer
    ///
    /// Storing a typed closure `Fn(&mut CTX, ...) -> ... + 'static` forces
    /// `CTX: 'static` through the closure's monomorphisation, which then
    /// cascades to any DB generic appearing in CTX. Since the `EvmFactory`
    /// trait method `create_evm<DB: Database>` does not bound `DB: 'static`
    /// (and impls cannot add stricter bounds than the trait), a
    /// CTX-generic closure cannot be installed from inside that method. A
    /// raw `*mut core::ffi::c_void` carries no type information and
    /// therefore imposes no lifetime bound on DB.
    ///
    /// # Safety contract (caller)
    ///
    /// The `dispatch` closure body MUST cast `ctx_ptr` back to the exact
    /// `*mut Context<...>` type that the [`PrecompileProvider`] is
    /// specialised for on this `PrecompilesMap` instance. In practice this
    /// holds by construction: a `PrecompilesMap` is built inside one
    /// EVM-factory `create_evm<DB>` call for one specific Context type,
    /// and the hook is registered there with that same type. Mismatched
    /// casts are undefined behaviour.
    pub fn set_ctx_dispatch_hook<F1, F2>(&mut self, handles: F1, dispatch: F2)
    where
        F1: Fn(&Address) -> bool + Send + Sync + 'static,
        F2: Fn(
                *mut core::ffi::c_void,
                &CallInputs,
            ) -> Result<Option<InterpreterResult>, String>
            ...
            + 'static,
    {
        self.ctx_dispatch = Some((Arc::new(handles), Arc::new(dispatch)));
    }

    /// Outbe fork extension. Returns whether a context-dispatch hook is
    /// installed. Visible for tests.
    pub fn has_ctx_dispatch_hook(&self) -> bool {
        self.ctx_dispatch.is_some()
    }
    ...
    DB: Database,
{
    ...
    fn run(
        ...
        // Outbe fork extension: consult the ctx-dispatch hook (if installed)
        // BEFORE the standard precompile lookup / `Context` destructuring.
        // The hook receives the unbroken `&mut Context` via a raw pointer
        // (type-restored to the original `CTX` inside the wrapper closure).
        // See `set_ctx_dispatch_hook` for the safety contract.
        if let Some((handles, dispatch)) = self.ctx_dispatch.as_ref() {
            if handles(&inputs.bytecode_address) {
                let ctx_ptr = context as *mut _ as *mut core::ffi::c_void;
                return dispatch(ctx_ptr, inputs);
            }
        }
